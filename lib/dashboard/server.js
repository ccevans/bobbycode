...
import { listTickets, findTicket, createTicket, updateTicket, addComment, listEpics, getFeatureTickets } from '../tickets.js';
...
export function buildServer({
  ...
  route('GET', '/api/tickets/:id', (req, res, params) => {
    ...
    sendJson(res, 200, { ticket: { ...found.data, content: found.content, path: found.path } });
  });
  ...
  route('GET', '/api/features/:id', (req, res, params) => {
    try {
      ...
      sendJson(res, 200, {
        epic: { ...epic.data, content: epic.content },
        children,
      });
    } catch (e) {
  ...
  /**
   * Live feature progress: child ticket stages read from the workspace's
   * WORKTREE. While a feature runs, children advance in the worktree — the
   * main checkout's tickets (and /api/features/:id) don't see it until merge.
   */
