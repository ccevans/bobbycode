...
export function buildServer({
  ...
  // orchestrator stub.
  const boardDir = () => (orchestrator && orchestrator.ticketsDir) || ticketsDir;
  ...
  /**
   * The workspaces the ACTIVE project owns (TKT-022). A workspace belongs to the
   * active project when its ticket is on the active board — membership is by
   * board, not ticket-prefix, so two projects that happen to share a prefix do
   * not bleed into each other's list. Repo runs (no ticket, they act on the main
   * checkout) and pre-context hosts are never filtered. Off-studio the whole
   * list passes through unchanged.
   */
  const scopeToProject = (workspaces) => {
    const pc = projectContext();
    if (!pc || !pc.isStudio() || !pc.projectName) return workspaces;
    const dir = boardDir();
    if (!dir) return workspaces;
    return workspaces.filter((ws) => !ws.ticketId || !!findTicket(dir, ws.ticketId));
  };
  ...
  route('GET', '/api/brief', (req, res) => {
    try {
      const brief = buildBrief(boardDir(), resolvedSprintsDir);
  ...
  route('POST', '/api/go', async (req, res) => {
    try {
      ...
      const argv = body.argv || buildBrief(boardDir(), resolvedSprintsDir).nextAction.argv;
      const result = await executeGoAction(argv, { orchestrator, ticketsDir: boardDir() });
  ...
  route('GET', '/api/tickets/:id', (req, res, params) => {
    const found = findTicket(boardDir(), params.id);
  ...
  route('POST', '/api/tickets', async (req, res) => {
    try {
      ...
      const created = createTicket(boardDir(), {
      ...
      const found = findTicket(boardDir(), created.id);
  ...
  route('POST', '/api/tickets/:id/move', async (req, res, params) => {
    try {
      ...
      moveWithAlias(boardDir(), params.id, body.stage, { reason: body.reason || '' });
      const found = findTicket(boardDir(), params.id);
  ...
  route('PATCH', '/api/tickets/:id', async (req, res, params) => {
    try {
      ...
      updateTicket(boardDir(), params.id, updates);
      const found = findTicket(boardDir(), params.id);
      sendJson(res, 200, { ticket: { ...found.data, content: found.content } });
    } catch (e) {
      sendError(res, 400, e.message);
  ...
  route('POST', '/api/tickets/:id/comments', async (req, res, params) => {
    try {
      ...
      addComment(boardDir(), params.id, 'app', body.text);
      const found = findTicket(boardDir(), params.id);
      sendJson(res, 200, { ticket: { ...found.data, content: found.content } });
    } catch (e) {
      sendError(res, 400, e.message);
  ...
  route('GET', '/api/features', (req, res) => {
    try {
      sendJson(res, 200, { features: listEpics(boardDir()) });
    } catch (e) {
      sendError(res, 500, e.message);
    }
  });
  ...
  route('GET', '/api/features/:id', (req, res, params) => {
    try {
      const { epic, children } = getFeatureTickets(boardDir(), params.id);
      sendJson(res, 200, {
  ...
  route('GET', '/api/workspaces', (req, res) => {
    sendJson(res, 200, { workspaces: scopeToProject(store.list()) });
  });
