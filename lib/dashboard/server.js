...
export function buildServer({
  ...
  ideasFile = null,
  // ChatManager instance for conversational planning (TKT-021). Created
  // internally when not provided, using the same store and orchestrator.
  chatManager: chatManagerOpt = null,
}) {
  const routes = [];
  ...
  const chatsFile = path.join(repoRoot || '.', (config && config.bobby_dir) || '.bobby', 'chats.json');
  const chatManager = chatManagerOpt || new ChatManager({ store, orchestrator, chatsFile });
  ...
  route('GET', '/api/chats/:id', (req, res, params) => {
  ...
  route('POST', '/api/chats', async (req, res) => {
    try {
      const body = await readBody(req);
      if (!body.ticketId) return sendError(res, 400, 'ticketId is required');
      const result = await chatManager.startChat(body.ticketId);
      sendJson(res, 200, { chatId: result.chatId, ticketId: result.ticketId, status: 'active' });
    } catch (e) {
      sendError(res, 400, e.message);
    }
  });

  route('POST', '/api/chats/:id/message', async (req, res, params) => {
    try {
      const body = await readBody(req);
      if (!body.message) return sendError(res, 400, 'message is required');
      await chatManager.sendMessage(params.id, body.message);
      const chat = chatManager.getChat(params.id);
      sendJson(res, 200, { chat });
    } catch (e) {
      const status = /not found/i.test(e.message) ? 404 : 400;
  ...
  route('POST', '/api/chats/:id/commit', async (req, res, params) => {
    try {
      await chatManager.commitPlan(params.id);
      const chat = chatManager.getChat(params.id);
      sendJson(res, 200, { chat });
    } catch (e) {
      const status = /not found/i.test(e.message) ? 404 : 400;
