...
import { onboardStudio } from './onboard.js';
...
export function buildServer({
   ...
   * to the active project exactly as before. An unknown project throws here and
   * is converted to a 400 naming the real ones at ONE place — the dispatcher,
   * before any handler runs — because route-level catches each turned it into
   * their own 500 (the TC-10 live rejection).
   */
  const requestScope = (req) => {
    ...
    return pc.resolve(name);   // throws on an unknown project — the dispatcher 400s it
  };
  const boardDir = (req) => {
  ...
  route('GET', '/api/projects', (req, res) => {
    ...
    // Names alone made the App's picker spec undeliverable: its rows promise an
    // open-ticket count and a liveness dot, and nothing served them (BOB-067
    // design-check #3). `open` = tickets not done; `running` = a live agent on
    // that project's board. Both best-effort — a project whose board cannot be
    // read still lists, with the facts it cannot answer left null/false.
    const live = new Set(store.list()
      .filter((w) => w.status === 'running' && w.project)
      .map((w) => w.project));
    const projects = pc.listProjects().map((name) => {
      let open = null;
      try {
        open = listTickets(path.join(repoRoot, '.bobby', name, 'tickets'))
          .filter((t) => t.stage !== 'done').length;
      } catch { /* unreadable board: absent is not zero */ }
      return { name, open, running: live.has(name) };
    });
    sendJson(res, 200, { projects, active: pc.projectName });
  });

  // Idea -> repo + studio project + first ticket (BOB-024). Composed from the
  // studio primitives — never lib/project.js createProject, whose per-repo
  // .bobbyrc.yml would shadow the studio (the BOB-117 failure, rebuilt).
  route('POST', '/api/onboard', async (req, res) => {
    try {
      const body = await readBody(req);
      const result = onboardStudio(repoRoot, { idea: body.idea, stack: body.stack });
      // Land the caller on the new project — but never by flipping the global
      // out from under someone else (review B1). A header-addressed request is
      // a phone using BOB-067's stateless routing: it will ADDRESS the new
      // project via transport.project, so flipping the desktop's board here is
      // exactly the race that machinery exists to prevent. The global flips
      // only for a local caller, or when there is nothing to flip away from —
      // the empty studio's first project, where gaining an active project is
      // the point.
      const addressed = !!req.headers['x-bobby-project'];
      const pc = projectContext();
      if (!addressed || !pc?.projectName) orchestrator.switchProject?.(result.project);
  ...
  route('GET', '/api/config', (req, res) => {
    ...
    sendJson(res, 200, {
      ...
      // `claude-code` is the default TARGET — a lib/targets adapter the App
      // reads back — not an executor flavor. BOB-136 swept `lib/dashboard/` for
      // hardcoded executor names; this hit is deliberately not one of them.
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      for (const r of routes) {
        ...
        // BOB-067: validate the request's project addressing BEFORE the handler
        // runs. requestScope throws on a project this studio does not serve;
        // converting it here — the one place every API route passes through —
        // gives every board-scoped route the same clean 400 naming the real
        // projects, instead of each handler's catch turning it into its own 500
        // (the TC-10 live rejection). Only the typed unknown-project failure
        // converts: a valid project whose config fails to read is a genuine
        // server-side error and falls through to the 500 below.
        try {
          requestScope(req);
        } catch (e) {
          if (e?.code !== 'UNKNOWN_PROJECT') throw e;
          return sendError(res, 400, e.message);
        }
