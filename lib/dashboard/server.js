...
export function buildServer({
  ...
  const sessionsBoardDir = () => (orchestrator && orchestrator.sessionsDir)
  ...
  route('POST', '/api/tickets', async (req, res) => {
    try {
      ...
      const created = createTicket(boardDir(), {
        prefix: activeConfig().ticket_prefix || 'TKT',
        title: body.title,
  ...
  route('GET', '/api/workflows', (req, res) => {
    ...
    sendJson(res, 200, {
      ...
      workflows: listWorkflows(activeConfig() || {}),
      stages: describeWorkflows(activeConfig() || {}),
    });
  });
  ...
  route('GET', '/api/config', (req, res) => {
    ...
    sendJson(res, 200, {
      ...
      project: activeConfig().project || path.basename(repoRoot || ''),
      repo: originRepo(repoRoot),
      stack: activeConfig().stack || null,
      target: activeConfig().target || 'claude-code',
      stages: [...STAGES],
  ...
  route('GET', '/api/sessions', (req, res) => {
    try {
      const sessions = listSessions(sessionsBoardDir());
  ...
  route('GET', '/api/sessions/:id', (req, res, params) => {
    try {
      const entries = readSession(sessionsBoardDir(), params.id);
