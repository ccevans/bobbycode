...
export class ProjectContext {
  ...
  /**
   * Resolve a project WITHOUT switching to it (BOB-067).
   *
   * The tunnel routes each phone request to the project named on its frame.
   * Doing that with switchTo() would mutate the one global context — the
   * desktop and a second phone would have the board flipped under them, and a
   * frame for project B arriving before project A's proxied request is served
   * would mis-scope it. This returns a snapshot instead: same validation, same
   * cascade, no mutation, no persistence.
   *
   * @returns {{ project: string, config: object, ticketsDir: string, sessionsDir: string }}
   */
  resolve(name) {
    if (!this._studio) throw new Error('Per-project resolution is only available in a studio.');
    const projects = listStudioProjects(this.root);
    if (!projects.includes(name)) {
      // Typed so the API dispatcher can 400 THIS failure — the caller named a
      // project that does not exist, a client error — without also converting
      // the genuine server-side throws below it (a valid project whose config
      // fails to read stays a 500).
      const err = new Error(`No such project '${name}'. Studio projects: ${projects.join(', ') || '(none)'}.`);
      err.code = 'UNKNOWN_PROJECT';
      throw err;
    }
    return {
      project: name,
      config: configForProject(this.root, name),
      ticketsDir: path.join(this.root, '.bobby', name, 'tickets'),
      sessionsDir: path.join(this.root, '.bobby', name, 'sessions'),
    };
  }
