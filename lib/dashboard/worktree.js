...
/**
 * True if `child` is `parent` itself or a descendant of it. Both are
 * canonicalized first so symlinked temp dirs (macOS) compare correctly.
 */
function isPathUnder(child, parent) {
  const c = canonicalizeForContainment(child);
  const p = canonicalizeForContainment(parent);
  if (c === p) return true;
...
export function resolveWorktreeRoot(repoRoot, config, studioRoot = null) {
  ...
  if (config?.studio && studioRoot && !isPathUnder(resolved, studioRoot)) {
    throw new Error(
      `dashboard.worktree_root resolves outside the studio, so agents in its ` +
      `worktrees cannot reach the studio board.\n` +
      `  worktree_root: ${resolved}\n` +
      `  studio root:   ${studioRoot}\n` +
      `Set dashboard.worktree_root to a path UNDER the studio root ` +
      `(or leave it unset to use the default '../bobby-wt', a studio descendant).`
    );
  }
