...
export function resolveWorktreeRoot(repoRoot, config) {
  // NOTE (PRO-026): in studio mode, `bobby ticket move/view/assign` run from a
  // worktree only reach the studio board because findProjectRoot walks UP to it
  // — so worktree_root MUST resolve to a location under the studio root. The
  // default (`../bobby-wt`, a studio descendant) satisfies this; a worktree_root
  // outside the studio would break board writes. See the PRO-026 follow-up.
