...
/**
 * Canonicalize a path for containment checks: realpath the longest prefix that
 * exists on disk (so macOS /var → /private/var and other symlinks resolve), then
 * re-append the not-yet-created tail. `worktree_root` usually does not exist yet,
 * so a plain realpathSync would fail — this keeps the comparison symlink-safe
 * anyway.
 */
function canonicalizeForContainment(p) {
  ...
  try { cur = fs.realpathSync(cur); } catch { /* keep resolved form */ }
 ...
 *
 * STUDIO GUARD (PRO-027): in a studio, `bobby ticket move/view/assign` run from a
 * worktree reach the studio board ONLY because findProjectRoot walks UP from the
 * worktree cwd to the studio root — which holds only while the worktree is a
 * filesystem descendant of the studio root. The default (`../bobby-wt`, a studio
 * descendant) satisfies this; a `worktree_root` configured outside the studio
 * would silently write stage moves to the wrong board (or fail to find one). So
 * when `studioRoot` is known and the config is a studio, we REFUSE a worktree_root
 * that lands outside it — a clear config error beats a silent mid-run failure,
 * mirroring how the codebase refuses over-cap runs and lost main-checkout locks.
 * Non-studio (v1) projects are unaffected: no `studio` key, no validation.
 */
export function resolveWorktreeRoot(repoRoot, config, studioRoot = null) {
  const cfg = config?.dashboard?.worktree_root || '../bobby-wt';
  const resolved = path.resolve(repoRoot, cfg);

  if (config?.studio && studioRoot) {
    // THE REAL INVARIANT is not containment — it is that walking UP from the
    // worktree finds the STUDIO's .bobbyrc.yml first, because findProjectRoot
    // returns on the first hit. Containment was necessary but not sufficient,
    // and the gap is reachable with an ordinary config: a code repo that still
    // carries a leftover v1 .bobbyrc.yml (this studio's own repos did, until the
    // BOB migration retired them) plus `worktree_root: ./wt` inside that repo
    // passes a containment check and still resolves every board write to the
    // code repo's own board — the exact silent-wrong-board failure this guard
    // exists to prevent.
    //
    // Asking findProjectRoot directly subsumes the containment check: anything
    // outside the studio cannot resolve TO the studio either.
    // findProjectRoot THROWS when nothing above the path is a Bobby project, so
    // the "no config at all" case has to become a value here or its own error
    // replaces the actionable one.
    let reached = null;
    try { reached = findProjectRoot(resolved); } catch { reached = null; }
    // Canonicalized on both sides: on macOS a tmp path is /var/... while
    // realpath gives /private/var/..., and a raw string compare would reject a
    // perfectly good studio. This is what canonicalizeForContainment was written
    // for, and the reason it survives the containment check it used to serve.
    const sameRoot = reached !== null
      && canonicalizeForContainment(reached) === canonicalizeForContainment(studioRoot);
    if (!sameRoot) {
      throw new Error(
        `dashboard.worktree_root resolves somewhere agents cannot reach the studio board.\n` +
        `  worktree_root: ${resolved}\n` +
        `  studio root:   ${studioRoot}\n` +
        `  would resolve: ${reached || '(no .bobbyrc.yml above it at all)'}\n` +
        (reached && reached !== studioRoot
          ? `A .bobbyrc.yml between the two shadows the studio — a leftover single-project\n` +
            `config in the code repo will do it. Remove it, or point worktree_root elsewhere.\n`
          : '') +
        `Set dashboard.worktree_root to a path under the studio root with no other\n` +
        `.bobbyrc.yml between (or leave it unset to use the default '../bobby-wt').`
      );
    }
  }
 ...
 *
 * `studioRoot` (optional) is forwarded to `resolveWorktreeRoot` so the studio
 * guard fires here — at worktree creation, before any agent is spawned.
 */
export function computeWorktreePlacement(repoRoot, config, ticketId, stage = 'work', studioRoot = null) {
  const root = resolveWorktreeRoot(repoRoot, config, studioRoot);
  const dir = `${ticketId}-${slug(stage)}`;
  const worktreePath = path.join(root, dir);
  const worktreePrefix = config?.git_conventions?.worktree_prefix || 'bobby';
  const branch = `${worktreePrefix}/${slug(ticketId)}-${slug(stage)}`;
...
/**
 * The uncommitted diff of a checkout against its own HEAD.
 *
 * This is what a REPO RUN produces: an agent with no worktree edits the main
 * checkout in place, so there is no branch to compare and `diffAgainstMain`
 * would show nothing. Untracked files are absent by construction — `git diff`
 * cannot show content git has never seen, and `add -N` would mutate the user's
 * index to fake it. `workingChangedFiles` lists them instead.
 */
export function workingDiff(repoRoot, { maxBytes = 500000 } = {}) {
  let diff;
  try {
    diff = git(repoRoot, 'diff HEAD');
  } catch {
...
/**
 * Files changed in a checkout's working tree, in the same shape as
 * `changedFiles`. Tracked edits carry line counts; untracked files are listed
 * with null counts, the same way a binary file is — present, uncountable.
 */
export function workingChangedFiles(repoRoot) {
  const files = [];
  try {
    const raw = git(repoRoot, 'diff --numstat HEAD');
    ...
    // No commits yet — every file is untracked, handled below.
  }
  try {
    const untracked = git(repoRoot, 'ls-files --others --exclude-standard');
    for (const file of untracked.split('\n')) {
      if (file) files.push({ file, added: null, removed: null, untracked: true });
    }
  } catch { /* not a repo — the caller's git calls will have failed already */ }
  return files;
}
...
/**
 * The commit a directory is sitting on, or null if git cannot say — no repo, no
 * commits yet, or the directory is gone.
 *
 * Null is the honest answer and callers must treat it as "unknown", not as
 * "nothing changed": comparing this before and after a run is how the
 * orchestrator decides whether an agent wrote anything at all (TKT-062), and a
 * verdict that important must never be inferred from a failed git call.
 */
