...
/**
 * The Claude session id a stream event carries, or null (TKT-021).
 *
 * `--resume` needs the CLI's OWN session id — the one it prints on its
 * stream-json events (`session_id`), not the `ses-YYYYMMDD-…` id bobby generates
 * for its session log. The chat manager reads this off the first turn's events
 * and stores it so subsequent turns can resume the same conversation.
 *
 * Shaped for the claude CLI, whose events all carry `session_id` at the top
 * level of the JSON payload. A CLI that never emits one simply yields null, and
 * the conversation runs without resume (degraded but functional).
 */
 ...
 *   - resume         optional Claude session id passed as --resume (claude only)
 */
