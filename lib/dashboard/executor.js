...
//
// The binary and the flavor are two separate settings (BOB-137).
// `dashboard.executor` is the binary — a bare name or an absolute path — and
// the flavor whose flags drive it is `dashboard.executor_flavor` if set, else
// the binary's own basename. A binary Bobby cannot place is REFUSED, never
// approximated with claude's flags; see `resolveFlavor`.
//
...
/** The registered flavors, listed for a message a user has to act on. */
const knownFlavors = () => EXECUTOR_NAMES.join(', ');

/**
 * The CLI flavor a configured executor speaks (BOB-137).
 *
 * `dashboard.executor` names a BINARY; this decides which argv builder to drive
 * it with. Precedence: an explicit `dashboard.executor_flavor`, then the
 * binary's own basename (lowercased, extension stripped).
 *
 * It THROWS rather than falling back to claude. It used to fall back, and
 * against a real non-claude binary that is not merely "unrecognized", that
 * fallback is actively wrong: opencode's `-p` is the basic-auth PASSWORD flag,
 * so claude's argv delivered the prompt as a password and the binary exited 1
 * on a usage dump with the prompt never seen (captured live against opencode
 * 1.18.22 — BOB-085 `test-evidence/argv-abs-path-footgun.log`). A wrong argv
 * that looks like a working config is worse than a refusal that names the
 * setting, so the no-match branch refuses and says which key to set.
 *
 * Inference is deliberately basename-only. Probing the binary (`--version`)
 * would put process spawning inside a resolver that runs at CLI startup and on
 * every launch, and a wrapper that prints another CLI's version string would be
 * mis-inferred SILENTLY — the exact failure class this function exists to kill.
 */
export function resolveFlavor(bin, declared) {
  // A relative path is refused before anything else, including a declared
  // flavor: `commandExists` already rejects relative paths on purpose (the
  // preflight resolves them against the repo root while agents are spawned with
  // cwd set to their own worktree), so a relative path can only ever ENOENT.
  // Both separators, not path.sep — a forward-slash path is still a path on Windows.
  //
  // `bin` is null when no binary was configured at all (flavor set alone). This
  // guard must not run then: it hardcodes `dashboard.executor`, and blaming an
  // unset key — with a remedy ("use an absolute path") that is never valid for
  // `executor_flavor` — is the F2 wart the reviewer and tester both caught. The
  // declared-flavor check below already refuses a path there, naming the right
  // key and the only remedy that works: a bare registered flavor name.
  //
  // The `bin &&` is stated rather than implied, and is a deliberate mutation
  // SURVIVOR (the F3 class): deleting it leaves the suite green, because
  // `/[/\\]/.test(null)` coerces to the string 'null' and short-circuits before
  // `path.isAbsolute(null)` can throw. Leaning on that coercion is an accident,
  // not a design — the null case is named here so it stays readable.
  if (bin && /[/\\]/.test(bin) && !path.isAbsolute(bin)) {
    throw new Error(
      `dashboard.executor is set to '${bin}', which is a relative path. Bobby spawns `
      + 'agents with cwd set to their own worktree, so a relative path resolves somewhere '
      + `different on every run. Use an absolute path, or a bare CLI name (${knownFlavors()}).`
    );
  }

  if (declared) {
    if (!EXECUTORS[declared]) {
      throw new Error(
        `dashboard.executor_flavor is set to '${declared}', which is not a CLI flavor Bobby `
        + `knows. Set it to one of: ${knownFlavors()} — or remove the key to infer the flavor `
  ...
  if (EXECUTORS[bin]) return bin;

  const base = path.basename(bin, path.extname(bin)).toLowerCase();
  if (EXECUTORS[base]) return base;

  throw new Error(
    `dashboard.executor is set to '${bin}', and Bobby cannot tell which CLI that is. It will `
    + "not guess: driving an unknown binary with claude's flags sends the prompt to whatever "
    + "'-p' means to that CLI (for opencode, -p is the basic-auth password), so the agent "
    + `never receives it. Set dashboard.executor_flavor to one of ${knownFlavors()} — or point `
    + 'dashboard.executor at a binary named after one of them.'
  );
}
 ...
 *
 * `dashboard.executor` is the BINARY and `dashboard.executor_flavor` is the
 * dialect it speaks — see `resolveFlavor`, which is where an unresolvable pair
 * is refused. The returned `bin` is whatever the user configured, path and all;
 * the returned `name` is the registered flavor, so the two can differ and every
 * caller must carry the bin rather than re-deriving it from the name.
 */
export function resolveExecutor(config = {}) {
  const explicit = config.dashboard?.executor;
  const declared = config.dashboard?.executor_flavor;
  if (explicit) {
    const name = resolveFlavor(explicit, declared);
    return { ...EXECUTORS[name], name, bin: explicit };
  }
  // A flavor named with no binary alongside it still decides the flavor. The
  // alternative is a key the user set that does nothing, which is the failure
  // class this ticket exists to remove.
  if (declared) {
    // No binary to place, so nothing is passed as one — handing `declared` in
    // as `bin` made a bad flavor blame `dashboard.executor`, a key that is by
    // definition unset on this branch (F2).
    const name = resolveFlavor(null, declared);
 ...
 *   - executor       a REGISTERED flavor name (EXECUTOR_NAMES; default 'claude').
 *                    Anything else throws — a binary path belongs in `claudeBin`,
 *                    with `resolveFlavor` deciding the dialect (BOB-137).
 *   - claudeBin      the resolved binary, which may be an absolute path. Wins
 *                    over the flavor's default bin. The legacy name is kept so
...
export function runAgent({
  ...
  // The second copy of the claude fallback deleted by BOB-137. It used to make
  // an unregistered name a "custom binary with claude-style flags"; leaving it
  // here would re-open the hole for every direct caller of runAgent, since the
  // orchestrator now passes a resolved flavor name and its bin separately.
  const flavor = EXECUTORS[executor];
  if (!flavor) {
    throw new Error(
      `runAgent: '${executor}' is not a registered executor flavor (${knownFlavors()}). `
      + 'dashboard.executor names the binary; dashboard.executor_flavor names the CLI it speaks.'
    );
  }
