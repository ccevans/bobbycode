...
import { isStudio, addRepo, createProject as createStudioProject } from '../studio.js';
...
export function onboardStudio(root, { idea, stack } = {}) {
  ...
  if (fs.existsSync(repoDir) && fs.readdirSync(repoDir).length > 0) {
    throw new Error(`"${name}" already exists. Describe it a little differently, or clear repos/${name}.`);
  }
