...
// The project NAME is a greeting — Home's h1 says it back to the user — not a
// storage key, so it is built from WHOLE words: a hard `.slice(0, 40)` cut the
// idea mid-thought ('a-site-where-local-bakeries-list-day-old', design-check
// round 2 finding 3). Words are kept while they fit a short budget; the same
// idea always derives the same name, which is what keeps the collision
// message ("describe this one a little differently") honest.
const NAME_BUDGET = 30;
const slugify = (s) => {
  const words = String(s).toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim().split(' ').filter(Boolean);
  if (!words.length) return 'new-project';
  // A single word longer than any budget cannot be boundary-split — cap it hard.
  let name = words[0].slice(0, 40);
  for (const w of words.slice(1)) {
    if (name.length + 1 + w.length > NAME_BUDGET) break;
    name += `-${w}`;
  }
  return name;
};
...
export function onboardStudio(root, { idea, stack } = {}) {
  ...
  const { config: projectConfig } = createStudioProject(root, name, { repos: [name] });
  ...
  // The prefix is the one createStudioProject JUST wrote into the project
  // config: without it the id minted as `undefined-001` and Home's Next action
  // printed `bobby run workflow undefined-001` (design-check round 2 finding 2).
