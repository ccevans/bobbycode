...
import fs from 'fs';
import { buildPromptFor, resolveWorkflow } from '../workflow.js';
...
export class Orchestrator {
  ...
  _resolveNextAgent(ws) {
    // Use the workspace's own workflow to find the stage after the current one
    const pipeline = this._pipelineFor(ws);
    const idx = pipeline.findIndex(s => s.stage === ws.stage);
