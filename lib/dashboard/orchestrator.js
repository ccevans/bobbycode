...
import fs from 'fs';
import { buildPromptFor } from '../workflow.js';
...
export class Orchestrator {
  ...
  async runAgent(workspaceId, { agentOverride } = {}) {
    ...
    const hasProduct = fs.existsSync(path.join(this.repoRoot, this.config.bobby_dir || '.bobby', 'product', 'feature-map.md'));
    let epicData;
    if (agent === 'feature') {
      try {
