...
import fs from 'fs';
import { buildPromptFor, resolveWorkflow, resolveNextAgent, assertTicketFree } from '../workflow.js';
...
import { runAgent, resolveExecutor, executorLabel, resolvePermissionMode, isPermissionDenial, sessionIdFromEvent } from './executor.js';
...
import { claimTicket, claimFileForTicket, releaseTicketClaim, CLAIM_TOKEN_ENV } from './ticket-claim.js';
...
export class Orchestrator {
  ...
  _configFor(ws) {
    ...
    try {
      ...
      // The run's own project was renamed, removed, or its .bobbyrc.yml
      // corrupted while it was in flight. Neither the live config nor this one
      // is the run's project, so fall back to the BOOT config: immutable, always
      // valid, and — when the run's project IS the boot project, the common case
      // — exactly right. Throwing here instead would strand the workspace as
      // `running` forever inside the exit handler.
      return this._config;
    }
  }
  ...
  createWorkspace({ ticketId, agent = 'plan', pipelineName }) {
    ...
    // One live run per ticket, across processes (BOB-120). Taken BEFORE the
    // worktree so a refusal costs nothing, and before the record exists so a
    // refused start leaves no trace. Without it, tapping "Start work" on a
    // ticket a background agent was already running simply made a second
    // workspace on the same branch — the two runs then overwrite each other.
    // The claim is a file on the board because the other orchestrator is a
    // different process: `bobby app`, a terminal `bobby run`, and a background
    // agent share no memory, only the board.
    // A feature run drives the epic's CHILDREN, so the epic being free says
    // nothing about whether the work is. Checked HERE, before any side effect:
    // refusing at prompt-build time instead left a cut worktree, a stored
    // workspace record and a held epic claim behind, and named neither ticket.
    if (agent === 'feature') {
      let children = [];
      try { ({ children } = getFeatureTickets(this.ticketsDir, ticketId)); } catch { /* not an epic: runAgent reports it */ }
      // assertTicketFree, not a hand-rolled copy of it. The copy re-introduced
      // the raw EACCES this ticket was rejected for one round earlier: an
      // unreadable run.lock on ANY child killed the feature run with a
      // filesystem error. Three copies of one rule is how that drift happens.
      for (const child of children) assertTicketFree(this.ticketsDir, child.id, child);
    }

    const claimFile = claimFileForTicket(ticket);
    const claim = claimTicket(claimFile, { holder: `bobby app (${agent})` });

    // Everything from here can throw — an unknown workflow, a two-repo ticket, a
    // worktree_root outside the studio. Each must release the claim, or a
    // refused start would leave the ticket looking permanently in progress until
    // the staleness ceiling expired.
    try {
    ...
    // The claim is held for the WORKSPACE's life, not one run's: the collision
    // this prevents is two workspaces on one branch, and a workspace outlives
    // the run that created it. The token rides on the record so whoever tears
    // the workspace down can release it, and only it — a stale claim that was
    // already reclaimed must not be deletable by the corpse.
    workspace.claim = { file: claimFile, token: claim.record.token };
   ...
   * on. Both resume the same CLI conversation via the executor's resume argv
   ...
   * exit handler records the turn, captures the CLI's own session/thread id
   * from the stream (sessionIdFromEvent — claude `session_id`, codex
   * `thread_id`; TKT-021, BOB-133) so the next turn can resume, and appends to
   * the timeline.
   */
  ...
  _launch(ws, { agent, prompt, worktreePath, ticketIds, permissionMode, resume, onEvent, onExit }) {
    ...
    // Resolve the executor FIRST, before anything is mutated (BOB-137).
    // `resolveExecutor` now refuses a `dashboard.executor` whose CLI flavor it
    // cannot place, and a throw further down — after initSession, the running
    // mark and the run_start broadcast — would strand the workspace in
    // `running` forever with no process. Nothing between here and the launch
    // reads either of these, so the hoist is behaviour-neutral on the happy path.
    ...
    // Launch executor (resolved at the top of this method).
    // Named once, here, from the run's OWN pinned config, and carried to the
    // exit on the result (BOB-136). Re-resolving at exit would be a live config
    // read on a run whose config is pinned, in a handler where a throw strands
    // the workspace in `running` forever.
    const executorName = executorLabel(executor);
    const handle = this._runExecutor({
      worktreePath,
      prompt,
      sessionId,
      executor: executor.name,
      // The BINARY, alongside the flavor name (BOB-137). Since a path-configured
      // executor resolves to a registered flavor whose default bin is the bare
      // name, `runAgent` would otherwise re-derive `opencode` from the registry
      // and spawn the very binary that is not on PATH — ENOENT on every run, for
      // exactly the users this rule exists to help. For a bare-name config
      // `bin === name`, so nothing else changes.
      claudeBin: executor.bin,
      // Per AGENT, not one model for the whole board (BOB-135). The review of a
      // diff and the recording of a page's load time are not the same kind of
      // work and should not cost the same. resolveModel still honours an
      // explicit `dashboard.model` above Bobby's own tiers, so a project that
      // already pinned one model keeps running on it.
      model: resolveModel(agent, runConfig, { acceptsTiers: !!executor.tierAliases }),
      ...
      // discussing, then the launch posture to commit the plan) and pass the
      // CLI's own session/thread id to resume (claude `--resume <session_id>`,
      // codex `exec resume <thread_id>`) — TKT-021, BOB-133.
      ...
      // Same precedence chain, for the ticket claim (BOB-120). The agent runs
      // `bobby run <stage> <its own ticket>`, which checks the claim; without the
      // token it is a different process with no way to prove the claim is its
      // own orchestrator's, and its own run refuses it.
      env: {
        ...(ws.project ? { BOBBY_PROJECT: ws.project } : {}),
        ...(ws.claim && ws.claim.token ? { [CLAIM_TOKEN_ENV]: ws.claim.token } : {}),
      },
      resume,
      onEvent: (ev) => {
    ...
    //
    // Both branches settle a result STAMPED with the executor that ran, so
    // every exit path — including the catch below, which builds its own result
    // — can name it without re-resolving. This orchestrator's resolution is the
    // authority, so the stamp goes on last.
    const settle = (result) => {
      const stamped = { ...result, executor: executorName };
      if (!onExit) {
        return this._onExit(workspaceId, agent, sessionId, stamped, { headAtStart });
      }
      this.runningProcesses.delete(workspaceId);
      this.permissionDenials.delete(workspaceId);
      return onExit(stamped);
    };
   ...
   *
   * Names the executor that ACTUALLY ran (BOB-136) — this line hardcoded
   * `claude` from 6c01c85 until the dashboard grew four flavors, and a failed
   * opencode run sent its user to the wrong binary, docs and auth.
   * `result.executor` is stamped by `_launch`'s `settle` from the run's own
   * resolved executor; it is never re-resolved here, and the fallback keeps a
   * caller that bypasses `_launch` honest rather than saying `undefined`.
   */
  _lastErrorFor(result) {
    if (result.exitCode !== 0 && result.exitCode !== null) {
      return `${result.executor || 'the agent CLI'} exited with code ${result.exitCode}`;
    }
  ...
  async merge(workspaceId, { message } = {}) {
    ...
    // The work landed, so the ticket is free for whoever comes next (BOB-120).
    // Released here rather than on run exit because the claim covers the
    // workspace, and an un-merged workspace still owns the branch.
    this._releaseTicketClaim(ws);
   ...
   */
  /**
   * Drop this workspace's ticket claim, if it still holds one (BOB-120).
   *
   * Token-scoped: a workspace whose claim was reclaimed as stale must not be
   * able to release the claim that replaced it. Silent when there is nothing to
   * release — workspaces created before claims existed have no token, and a
   * double release is a no-op, mirroring _releaseRepoLock.
   */
  _releaseTicketClaim(ws) {
    if (!ws || !ws.claim || !ws.claim.file || !ws.claim.token) return false;
    try {
      return releaseTicketClaim(ws.claim.file, ws.claim.token);
    } catch {
      return false;   // a board that moved out from under us must not fail a merge
    }
  }
  ...
  async discard(workspaceId, { force = false } = {}) {
    ...
    // Discard IS a normal end (BOB-120). Without this the run.lock outlives the
    // workspace holding a live app-server pid, so it is not stale, the UI no
    // longer shows the workspace it names, and the ticket is refused to every
    // orchestrator until the 6h ceiling — with nothing in the product able to
    // clear it.
    this._releaseTicketClaim(ws);
