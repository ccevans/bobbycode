...
import { newWorkspace, newRepoRun, isRepoRun, makeWorkspaceId, makeRepoRunId } from './state.js';
...
export class Orchestrator {
  ...
  _terminalStatus(result) {
    if (result.exitCode === 0) return 'idle';
    if (result.signal === 'SIGTERM' || result.signal === 'SIGKILL') return 'stopped';
    return 'failed';
  }
