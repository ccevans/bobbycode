...
export class Orchestrator {
  ...
  createWorkspace({ ticketId, agent = 'plan', pipelineName }) {
    ...
    const workspace = newWorkspace({
      ...
      repoRoot,
      lockFile,
      // The board this workspace was created FROM, pinned for the life of the
      // record so a later project switch cannot move it (TKT-022).
      ticketsDir: this.ticketsDir,
      sessionsDir: this.sessionsDir,
    });
  ...
  createRepoRun({ agent }) {
    ...
    const run = newRepoRun({ id, agent, pipeline: this.pipelineName, sessionsDir: this.sessionsDir });
  ...
  runChatTurn(workspaceId, { message, mode = 'plan' } = {}) {
    ...
    const permissionMode = mode === 'commit'
      ? resolvePermissionMode(this.config, 'worktree')
      : 'plan';
  ...
  _launch(ws, { agent, prompt, worktreePath, ticketIds, permissionMode, resume, onEvent, onExit }) {
    ...
    const handle = this._runExecutor({
      ...
      permissionMode: permissionMode || resolvePermissionMode(this.config, kind),
      resume,
      onEvent: (ev) => {
