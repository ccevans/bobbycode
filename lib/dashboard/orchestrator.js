...
import { claimTicket, claimFileForTicket, releaseTicketClaim, readTicketClaim, isOwnClaim, claimedMessage, CLAIM_TOKEN_ENV } from './ticket-claim.js';
  ...
  createWorkspace({ ticketId, agent = 'plan', pipelineName }) {
    ...
    if (agent === 'feature') {
      ...
      for (const child of children) {
        const record = readTicketClaim(claimFileForTicket(child));
        if (record && !isOwnClaim(record)) throw new Error(`${child.id}: ${claimedMessage(record)}`);
      }
    }
