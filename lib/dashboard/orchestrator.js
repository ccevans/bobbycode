...
import { runAgent, resolveExecutor, resolvePermissionMode, isPermissionDenial } from './executor.js';
...
export class Orchestrator {
   ...
   */
  async runAgent(workspaceId, { agentOverride, resume, permissionMode } = {}) {
    ...
    // Chat overrides: resume (Claude session id for --resume) and
    // permissionMode (e.g. 'plan' for read-only chat turns). Passed through
    // _launch to the executor — see TKT-021.
    const launchOverrides = {};
    if (resume) launchOverrides.resume = resume;
    if (permissionMode) launchOverrides.permissionMode = permissionMode;

    return isRepoRun(ws)
      ? this._runInMainCheckout(ws, agent, launchOverrides)
      : this._runInWorktree(ws, agent, launchOverrides);
  }
   ...
   */
  _runInWorktree(ws, agent, launchOverrides = {}) {
   ...
   * never happens.
   */
  _runInMainCheckout(ws, agent, launchOverrides = {}) {
   ...
   */
  _launch(ws, { agent, prompt, worktreePath, ticketIds, resume, permissionMode: permissionOverride }) {
    ...
    const handle = this._runExecutor({
      ...
      // A chat turn may override this to 'plan' for read-only mode (TKT-021).
      permissionMode: permissionOverride || resolvePermissionMode(this.config, kind),
      onEvent: (ev) => this._onExecutorEvent(workspaceId, sessionId, ev),
      // --resume for conversation continuity (TKT-021). Null when not set.
    ...
    handle.done.then((result) => this._onExit(workspaceId, agent, sessionId, result, { headAtStart }))
      .catch((e) => this._onExit(workspaceId, agent, sessionId, { exitCode: null, signal: null, error: e.message }, { headAtStart }));
  ...
  /**
   * Run a chat turn: one agent invocation in plan (read-only) mode with
   * --resume for conversation continuity (TKT-021). Thin wrapper over
   * `runAgent` that sets the right options for a conversational planning
   * turn.
   *
   * @param {string} workspaceId
   * @param {object} opts
   * @param {string} [opts.resume] — Claude session id for --resume
   */
  async runChat(workspaceId, { resume } = {}) {
    return this.runAgent(workspaceId, {
      resume,
      permissionMode: 'plan',
    });
  }
  ...
  /**
   * Commit a chat: one final agent turn with --resume but WITHOUT the plan
   ...
   * @param {string} [opts.resume] — Claude session id for --resume
   */
  async commitChat(workspaceId, { resume } = {}) {
    return this.runAgent(workspaceId, { resume });
  }
