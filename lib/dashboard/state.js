 ...
 */
export function newRepoRun({ id, agent, pipeline = 'default', sessionsDir = null }) {
  return newWorkspace({
    id,
    ticketId: null,
    worktreePath: null,
    ...
    pipeline,
    kind: 'repo',
    // No ticket, so no board — but a repo run still writes a session log, and
    // that log belongs to the project it was launched from.
    sessionsDir,
  });
}
