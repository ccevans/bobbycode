...
/**
 * The Needs-you queue: a workspace has stopped and is waiting on a human.
 *
 * This MUST stay identical to `NEEDS_YOU` in the pro repo's
 * app/app/lib/store.js (BOB-101 made that the app's single definition, shared
 * by the Home view, the Board and the iPhone widget). The host reads the same
 * two statuses to decide when to wake the phone — if these drift, the push and
 * the screen it opens disagree about what is waiting.
 */
export const NEEDS_YOU_STATUSES = new Set(['awaiting_approval', 'ready_to_merge']);
...
export function newWorkspace({
  id, ticketId, worktreePath, branch, agent, pipeline = 'default', kind = 'worktree',
  repoRoot = null, lockFile = null, project = null, ticketsDir = null, sessionsDir = null,
}) {
  ...
  return {
    ...
    repoRoot,
    lockFile,
    // The BOARD this workspace belongs to, pinned once at creation (TKT-022).
    // In a studio the orchestrator's own `ticketsDir`/`sessionsDir` move when the
    // user selects another project — while this run is still going. A run takes
    // minutes, so everything it does after launch (the stage re-read on exit, the
    // session log tail, the mergedAt stamp) has to read the board it STARTED on,
    // not whatever the UI now shows. Null off-studio, on repo runs' ticket board,
    // and on records written before this field existed — consumers fall back to
    // the orchestrator's own dirs, which is exactly today's behavior.
    //
    // `project` is the NAME the two dirs were derived from, and the same name
    // the run's CONFIG is read from (_configFor). One pinned answer rather than
    // three parallel ones that could disagree: .bobbyrc.yml is per project, and
    // `project_repos` in it decides which repo the worktree is cut from.
    project,
    ticketsDir,
    sessionsDir,
    agent: agent || null,
    ...
    lastError: null,
    // CONVERSATIONAL PLANNING (TKT-021). A chat session is a workspace in `plan`
    // mode with `--resume` enabled — no separate entity. `chatMode` says this
    // record is one; `chatId` is the CLAUDE session id captured from the first
    // turn's stream and replayed via --resume on later turns; `chatHistory` is
    // the `{role, summary, at}` timeline the UI draws. All three are inert on an
    // ordinary workspace, and default off so every existing record is unchanged.
    chatMode: false,
    chatId: null,
    chatHistory: [],
  };
 ...
 */
export function newRepoRun({ id, agent, pipeline = 'default', project = null, ticketsDir = null, sessionsDir = null }) {
  return newWorkspace({
    id,
    ticketId: null,
    worktreePath: null,
    ...
    pipeline,
    kind: 'repo',
    // No ticket of its own — but a freeform agent is still handed the board in
    // its prompt, and it still writes a session log. Both belong to the project
    // the run was created for, as does the config that shapes its prompt.
    project,
    ticketsDir,
    sessionsDir,
  });
