...
export const AGENT_REGISTRY = {
  ...
  'define-data-model': {
    ...
    agentName: 'bobby-define-data-model',
    cowork: true,
    promptHeader: 'Run the bobby-define-data-model agent to state the entities, their relations, and who owns the truth for each — after journeys, before the feature map.',
    promptSteps: [
      'Read `.bobby/product/journeys.md` FIRST (plus `personas.md` and `brief.md`) — entities are derived from what journey steps store and show, never brainstormed. An entity no journey step touches does not exist.',
      'Write `.bobby/product/DATA-MODEL.md`: one section per entity — journey-cited fields, relations with cardinality, and a source-of-truth call per entity (this system / a named external system / the user).',
      'Interview only where ownership is genuinely unclear (tags: [Entity] [Relation] [Truth]; budget 3-5 questions).',
      'Your FINAL message is the gate: "Here is every entity and who owns the truth for it. Point at the one source-of-truth call that is wrong — or name the entity that is missing." The gate accepts "skip": comment the skip on the epic and move on without writing the artifact.',
    ],
  },
  'define-architecture': {
    label: 'Define Architecture',
    agentName: 'bobby-define-architecture',
    cowork: true,
    promptHeader: 'Run the bobby-define-architecture agent to record the forward architecture — the components that WILL exist — and its load-bearing decisions as ADRs.',
    promptSteps: [
      'Read `.bobby/product/DATA-MODEL.md` when present (its stage can be skipped) plus `journeys.md`; read `.bobby/architecture-wakeup.md` if the repo already exists — a conflict with the backward view is an ADR, never a silent contradiction.',
      'Write `.bobby/product/ARCHITECTURE.md` as the FORWARD view — components, boundaries, where each entity lives, integration seams. This is intent (what WILL exist), explicitly distinct from `bobby run arch`\'s backward view (what DOES exist).',
      'Record each load-bearing call with `bobby decision add --id <kebab-case-id> --fact "..." --why "..." --ticket {EPIC}` — never hand-edit `.bobby/decisions.yaml`. ARCHITECTURE.md cites the decision ids; it never restates them.',
      'Your FINAL message is the gate: "These are the decisions bobby-review will hold every ticket to from day one. Veto one now — or say they stand." The gate accepts "skip": comment the skip on the epic and move on.',
    ],
  },
  'define-features': {
    ...
    promptSteps: [
      ...
      'Read `.bobby/product/DATA-MODEL.md` when present and cut the map against it — a Must feature needing an entity absent from the model amends the model (with a Changelog line), never invents one locally.',
      ...
      'Then LOCK every artifact present in `.bobby/product/` (Locked/Status headers; six when nothing was skipped), commit `git add .bobby/product .bobby/decisions.yaml`, and move the epic: `bobby ticket move {EPIC} plan`.',
    ],
  },
  'define-mockups': {
