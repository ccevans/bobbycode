 ...
 *
 * TIERS. Each entry names the kind of model its work wants:
 *
 *   opus    judgment — deciding what to build, drawing the v1 line, finding the
 *           regression a diff does not admit to, critiquing a design
 *   sonnet  execution — writing the code the plan already specified, driving a
 *           browser, transcribing decisions into a spec
 *   haiku   bookkeeping — recording a page's load time, checking a deploy
 *           returned 200
 *
 * A tier is an ALIAS, not a model name, so these stay true as models are
 * replaced. lib/models.js turns one into the concrete `--model` an executor
 * gets, and only where the alias is real: cursor-agent, codex and opencode take
 * full model names, so tiers never reach them (see that file's header).
 *
 * These are Bobby's opinion and nothing more — any of them is overridden per
 * project by a `models:` block in .bobbyrc.yml.
 */
export const AGENT_REGISTRY = {
  // --- Standard workflow agents (use buildSingleAgentPrompt / buildBatchStagePrompt) ---
  plan:   { label: 'Bobby Plan',   agentName: 'bobby-plan', tier: 'opus' },
  build:  { label: 'Bobby Build',  agentName: 'bobby-build', tier: 'sonnet' },
  review: { label: 'Bobby Review', agentName: 'bobby-review', tier: 'opus' },
  test:   { label: 'Bobby Test',   agentName: 'bobby-test', tier: 'sonnet' },
  ...
  ship:     { label: 'Bobby Ship',     custom: true, freeform: true, tier: 'sonnet' },
  // The chaining agents run EVERY stage of a workflow in one process, so one
  // model has to serve all of them. Tiered to the most demanding link in the
  // chain (review, and planning an epic) rather than the average one — an
  // execution-tier model doing the review stage is the failure this split
  // exists to prevent.
  workflow: { label: 'Bobby Workflow', custom: true, tier: 'opus' },
  feature:  { label: 'Bobby Feature',  custom: true, tier: 'opus' },
  next:     { label: 'Bobby Next',     custom: true, tier: 'opus' },
