...
export const BUILT_IN_WORKFLOWS = {
  ...
  define: ['define-brief', 'define-personas', 'define-journeys', 'define-features'],
};
 ...
 */
export function buildSingleAgentPrompt(agent, ticketId, ticketsDir = '.bobby/tickets', agentsPath = '.claude/agents', hasServices = false, hasProduct = false) {
  let step = 3;
  ...
  const productHint = hasProduct
    ? `\n${step++}. Check the ticket's \`feature\`/\`persona\` frontmatter. If present, read \`.bobby/product/feature-map.md\` (that row and the journey step(s) it serves) and \`.bobby/product/personas.md\` — the work must satisfy that journey step for that persona.`
    : '';
...
${step + 1}. When complete, update the ticket stage using \`bobby ticket move\`.
...
export function buildPromptFor(agent, ticketIds, ctx) {
  ...
  if (reg.promptSteps) {
    ...
    if (reg.cowork && ids.length > 0) {
      ...
      prompt = buildSingleAgentPrompt(reg.agentName, ticketId, ticketsRelDir, agentsPath, hasServices, hasProduct);
