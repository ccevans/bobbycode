...
export function buildPromptFor(agent, ticketIds, ctx) {
  ...
  if (ids.length === 0) {
    ...
    const available = stageTickets.filter(t => !t.assigned && !ticketHasLiveRun(ticketsDir, t.id));
    if (available.length === 0) {
      if (stageTickets.length > 0) {
        const running = stageTickets.filter(t => ticketHasLiveRun(ticketsDir, t.id)).length;
        const detail = running > 0 ? `${stageTickets.length} assigned or already running` : `${stageTickets.length} assigned`;
        throw new Error(`No available tickets in "${stage}" stage (${detail}).`);
      }
  ...
  const ticketId = ids[0];
  if (!findTicket(ticketsDir, ticketId)) throw new Error(`Ticket ${ticketId} not found`);
  assertTicketFree(ticketsDir, ticketId);
  return {
    prompt: buildSingleAgentPrompt(reg.agentName, ticketId, ticketsPath, agentsPath, hasServices, hasProduct, nextStageForAgent(reg.agentName, workflow), productDir),
...
export function assertTicketFree(ticketsDir, ticketId) {
  const ticket = findTicket(ticketsDir, ticketId);
  if (!ticket) return;
