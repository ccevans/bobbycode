 ...
 */
export function buildSingleAgentPrompt(agent, ticketId, ticketsDir = '.bobby/tickets', agentsPath = '.claude/agents', hasServices = false, hasProduct = false, nextStage = null) {
  let step = 3;
...
export function buildPromptFor(agent, ticketIds, ctx) {
  ...
  if (reg.promptSteps) {
    ...
    if (reg.cowork && ids.length > 0) {
      ...
      prompt = buildSingleAgentPrompt(reg.agentName, ticketId, ticketsPath, agentsPath, hasServices, hasProduct, nextStageForAgent(reg.agentName, workflow));
