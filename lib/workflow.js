...
import { isTicketClaimed, readTicketClaim, isOwnClaim, claimedMessage, claimFileForTicket } from './dashboard/ticket-claim.js';
...
export function buildPromptFor(agent, ticketIds, ctx) {
  ...
  if (agent === 'feature') {
    ...
    // A feature run drives the CHILDREN, so checking only the epic's claim let a
    // feature hand out a prompt for tickets the app was already running (BOB-120
    // B3c). Refused on the first child that is held, naming it.
    for (const child of children) assertTicketFree(ticketsDir, child.id);
...
/**
 * Is a live run holding this ticket? (BOB-120)
 *
 * WHAT THE CLI CAN AND CANNOT DO HERE, stated plainly. `bobby run` BUILDS a
 * prompt and exits — the agent a human or a harness then runs is outside Bobby's
 * process — so there is nothing for the CLI to hold a claim FOR, and taking one
 * would release it immediately. This is the same reach limitation
 * main-checkout-lock.js documents about itself.
 *
 * So the CLI's half is to CHECK, not to claim: refuse to hand out a prompt for a
 * ticket the app is actively running. That covers the reported collision —
 * app run in flight, someone starts a CLI run on the same ticket — and it costs
 * nothing when the ticket is free. The app's orchestrator, which does hold a
 * process for the life of a workspace, is what takes the claim.
 */
export function ticketHasLiveRun(ticketsDir, ticketId) {
  const ticket = findTicket(ticketsDir, ticketId);
  if (!ticket) return false;
  try {
    return isTicketClaimed(claimFileForTicket(ticket));
  } catch {
    return false;   // an unreadable board must not block work
  }
}
...
export function assertTicketFree(ticketsDir, ticketId, known = null) {
  ...
  if (record && !isOwnClaim(record)) throw new Error(claimedMessage(record));
}
