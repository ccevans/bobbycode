...
export const BUILT_IN_WORKFLOWS = {
  ...
  // For projects with no live app to observe — CLIs, libraries, npm packages.
  // bobby-test verifies by exercising a running app and is forbidden from
  // running the spec suite, so it has nothing to do here and would block every
  // ticket. Review already runs the suite independently, which is the right
  // verification for a library. `bobby init` picks these automatically for
  // stacks that declare `default_workflow`.
 ...
 */
/**
 * The workflow a stack implies when the user hasn't chosen one.
 *
 * A stack declares it outright with `default_workflow`; otherwise it is
 * inferred, because a stack with nothing to serve and no way to start it has
 * no live app for the `test` stage to observe. Inference keeps custom stacks
 * working without requiring the new field.
 */
export function stackDefaultWorkflow(stack = {}) {
  if (stack.default_workflow) return stack.default_workflow;
  const hasHealthChecks = (stack.health_checks || []).length > 0;
  const hasDevCommand = !!(stack.commands && stack.commands.dev);
  return (!hasHealthChecks && !hasDevCommand) ? 'library' : 'default';
}
...
export function resolveWorkflow(config, name = 'default', ticketPipeline = null) {
  ...
  // then the ticket's own override, then the project-wide default.
  const projectDefault = config.default_workflow || 'default';
  const effectiveName = (name !== 'default')
    ? name
    : (ticketPipeline || projectDefault);
 ...
 */
// Agents that verify by observing a running application. On a project with no
// live app they have nothing to do — and their skills forbid falling back to
// the spec suite — so they must block loudly instead of emitting BLOCKED test
// cases one at a time while the ticket silently stops advancing.
export const LIVE_APP_AGENTS = [
  'bobby-test', 'bobby-qe', 'bobby-ux', 'bobby-pm', 'bobby-watchdog',
];

/**
 * True when the project has nothing for a live-app agent to observe.
 * Mirrors the inference in stackDefaultWorkflow, but reads the resolved config.
 */
export function hasNoLiveApp(config = {}) {
  const healthChecks = config.health_checks || [];
  const services = config.services || {};
  const devCommand = config.commands && config.commands.dev;
  return healthChecks.length === 0 && Object.keys(services).length === 0 && !devCommand;
}

export function buildSingleAgentPrompt(agent, ticketId, ticketsDir = '.bobby/tickets', agentsPath = '.claude/agents', hasServices = false, noLiveApp = false) {
  if (noLiveApp && LIVE_APP_AGENTS.includes(agent)) {
    return `The ${agent} agent cannot run on ticket ${ticketId}: this project has no live app.

\`${agent}\` verifies by exercising a running application and is forbidden from running the test suite as a substitute. This project declares no health checks, no services, and no dev command, so there is nothing for it to observe.

Do NOT emit BLOCKED test cases one by one, and do NOT fall back to running specs.

1. Block the ticket, stating the cause:
   \`bobby ticket move ${ticketId} block "no live app for ${agent} — switch to the library workflow"\`
2. Tell the user their project looks like a library or CLI, and that the fix is
   \`default_workflow: library\` in \`.bobbyrc.yml\` (or \`bobby run library ${ticketId}\` for one ticket). The library workflow ends at review, which runs the test suite.`;
  }
...
export function buildPromptFor(agent, ticketIds, ctx) {
  ...
  const noLiveApp = hasNoLiveApp(config);
  ...
  if (reg.promptSteps) {
    ...
    if (reg.cowork && ids.length > 0) {
      ...
      prompt = buildSingleAgentPrompt(reg.agentName, ticketId, ticketsRelDir, agentsPath, hasServices, noLiveApp);
