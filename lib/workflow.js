...
export function buildPromptFor(agent, ticketIds, ctx) {
  ...
  if (ids.length === 0) {
    ...
    const claims = new Map(stageTickets.map(t => [t.id, readTicketClaim(claimFileForTicket(t))]));
      ...
      if (stageTickets.length > 0) {
        ...
        if (running.length > 0) {
          ...
          throw new Error(
            ...
            `(${running[0].id} — ${first.holder}, started ${first.startedAt})` +
...
export function assertTicketFree(ticketsDir, ticketId, known = null) {
  ...
  if (!ticket) return;
  const record = readTicketClaim(claimFileForTicket(ticket));
