 ...
 */
export function buildSingleAgentPrompt(agent, ticketId, ticketsDir = '.bobby/tickets', agentsPath = '.claude/agents', hasServices = false, nextStage = null) {
...
export function buildNextStepPrompt(ticketId, workflow, ticketsDir, ticketsRelDir = '.bobby/tickets', agentsPath = '.claude/agents') {
  ...
  return buildSingleAgentPrompt(agent, ticketId, ticketsRelDir, agentsPath, false, nextStageForAgent(agent, workflow));
}
...
export function buildPromptFor(agent, ticketIds, ctx) {
  ...
  if (reg.promptSteps) {
    ...
    if (reg.cowork && ids.length > 0) {
      ...
      prompt = buildSingleAgentPrompt(reg.agentName, ticketId, ticketsRelDir, agentsPath, hasServices, nextStageForAgent(reg.agentName, workflow));
    } else if (reg.requiresTicket) {
      if (ids.length === 0) throw new Error(`Usage: bobby run ${agent} <ticketId>`);
      const ticketId = ids[0];
      if (!findTicket(ticketsDir, ticketId)) throw new Error(`Ticket ${ticketId} not found`);
      prompt = buildGenericPrompt(reg, {
        ticketId, ticketsDir: ticketsRelDir, agentsPath,
