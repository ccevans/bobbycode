...
import { readTicketClaim, isOwnClaim, claimedMessage, claimFileForTicket } from './dashboard/ticket-claim.js';
...
export const BUILT_IN_WORKFLOWS = {
  ...
  // No live app to exercise (CLI tools, libraries, npm packages), so the flow
  // ends at review — which runs the test suite independently, the correct
  // verification for a library. Selected automatically as `default_workflow`
  // when a project has no health checks and no dev command (BOB-090).
  library: ['plan', 'build', 'review'],
 ...
 *   3. User-defined `workflows.default` (or legacy `pipelines.default`) — an
 *      explicit override of the default workflow beats `default_workflow`,
 *      because a user who runs `bobby workflow add default …` must see it take
 *      effect; a stale stack-derived key silently winning would be a new
 *      silent failure (BOB-090)
 *   4. `config.default_workflow` — the project's derived default (a no-live-app
 *      project gets `library` at init)
 *   5. Built-in DEFAULT_WORKFLOW
 *
...
export function resolveWorkflow(config, name = 'default', ticketPipeline = null) {
  ...
  let effectiveName = (name !== 'default') ? name : (ticketPipeline || 'default');

  // `default_workflow` redirects only the NAME 'default', and only when the
  // user has not defined a default workflow of their own.
  const userDefined = config.workflows || config.pipelines || {};
  let fromDefaultWorkflowKey = false;
  if (effectiveName === 'default' && !userDefined.default && config.default_workflow) {
    effectiveName = config.default_workflow;
    fromDefaultWorkflowKey = true;
  }
  ...
  if (effectiveName !== 'default') {
    const hint = fromDefaultWorkflowKey
      ? ` (named by \`default_workflow:\` in .bobbyrc.yml — fix or delete that line)`
      : '';
    throw new Error(`Unknown workflow '${effectiveName}'${hint}. Available: ${Object.keys(workflows).join(', ')}`);
  }
  return DEFAULT_WORKFLOW;
}

/**
 * The workflow name a freshly initialized project should default to, or null
 * to leave the built-in default alone (BOB-090).
 *
 * An explicit `default_workflow` in the stack JSON wins outright. Otherwise a
 * project with no health checks and no dev command has no live app for the
 * test stage to exercise, so it gets `library` (plan → build → review).
 * Called at the two config-construction sites (`bobby init`, `bobby new`) —
 * deliberately NOT inside scaffoldProject, so refresh/re-scaffold never
 * rewrites config (BOB-123).
 */
export function deriveDefaultWorkflow(stack, config) {
  if (stack?.default_workflow) return stack.default_workflow;
  const noHealthChecks = (config?.health_checks ?? []).length === 0;
  const noDevCommand = !config?.commands?.dev;
  return (noHealthChecks && noDevCommand) ? 'library' : null;
}

/**
 * BOB-090: what to do when a ticket reaches `testing` but the project has no
 * live app to exercise. bobby-test is a live-app agent by contract (never runs
 * specs, never reads source), so with nothing to probe it can only emit
 * BLOCKED test cases one by one — a silent stall. This wording exists once so
 * every prompt (orchestration, sprint, feature, slow mode, single/batch test
 * runs) blocks the ticket loudly and names the fix instead.
 */
const NO_LIVE_APP_REASON = 'no live app to test — use the library workflow';

function testBlockInstruction(ticketRef) {
  return `this project has no \`health_checks\` configured — there is no live app for bobby-test to exercise. ` +
    `Do NOT launch a test subagent and do NOT emit per-case BLOCKED results. ` +
    `Instead run \`bobby ticket move ${ticketRef} block "${NO_LIVE_APP_REASON}"\`, report it, and continue to the next ticket. ` +
    `The fix: set \`default_workflow: library\` in .bobbyrc.yml (the library workflow ends at review, which runs the test suite), ` +
    `or add \`health_checks\` if this project does have a live app.`;
}
 ...
 */
/**
 * Resolve a ticket id to its on-disk folder name so prompts embed an openable
 * path instead of a `${id}*` glob the agent's file-read tool cannot expand.
 * Falls back to the glob when the folder can't be resolved (unknown id, or a
 * ticketsDir that isn't a readable path at build time) so default-path callers
 * and existing unit tests keep working with no regression.
 */
function resolveTicketFolder(ticketsDir, id) {
  try {
    const t = findTicket(ticketsDir, id);
    if (t && t.dirname) return t.dirname;
 ...
 */
function buildSubagentSteps(workflow, { ticketsDir, agentsPath, planReadPath = '', hasHealthChecks = true }) {
  ...
  return workflow.map((s, i) => {
    if (s.stage === 'testing' && !hasHealthChecks) {
      return `${i + 1}. If stage is "${s.stage}": ${testBlockInstruction('{TICKET_ID}')}`;
    }
    ...
    return `${i + 1}. If stage is "${s.stage}":${preGate}\n` +
      ...
      `         1. Load context: run \`bobby ticket view {TICKET_ID}\` (resolves the ticket's folder and prints its content — do not open the file by path).${planRead}\n` +
      ...
      `     ${k.verify}) After the subagent finishes, re-run \`bobby ticket view {TICKET_ID}\` and read the \`Stage:\` line to confirm the stage advanced.`;
  }).join('\n');
}
 ...
 */
function buildPreflightGates(firstStage, _ticketsDir = '.bobby/tickets') {
  return `**Pre-flight stage gates** — handle non-workflow stages before launching any subagent:
   ...
   - If stage is **"backlog"**: advance to the first workflow stage now — \`bobby ticket move {TICKET_ID} ${firstStage}\` — then re-run \`bobby ticket view {TICKET_ID}\` and continue with the new stage.`;
}
...
export function buildOrchestrationPrompt(ticketIds, workflow, maxRetries = 3, ticketsDir = '.bobby/tickets', maxIterations = 20, agentsPath = '.claude/agents', hasServices = false, gitConventions = {}, hasHealthChecks = true) {
  ...
  const steps = buildSubagentSteps(workflow, { ticketsDir, agentsPath, hasHealthChecks });
...
2. Read \`${ticketsDir}/${resolveTicketFolder(ticketsDir, ticketId)}/ticket.md\` to load the ticket context.${serviceHint}${productHint}
 ...
 */
export function buildNextStepPrompt(ticketId, workflow, ticketsDir, ticketsPath = '.bobby/tickets', agentsPath = '.claude/agents', hasHealthChecks = true) {
  ...
  if (agent === 'bobby-test' && !hasHealthChecks)
    return `Ticket ${ticketId} is in testing, but ${testBlockInstruction(ticketId)}`;
...
For each ticket, launch a subagent with isolation: "worktree" and this prompt:
  ...
  3. Load context: run \`bobby ticket view {ID}\`.
...
For each ticket, launch a subagent with this prompt:
  ...
  2. Load context: run \`bobby ticket view {ID}\`.
...
export function buildPromptFor(agent, ticketIds, ctx) {
  ...
  // BOB-090: service health checks are already aggregated into the top-level
  // key at init — do not re-aggregate here. No health checks means no live app
  // for the test stage, so testing prompts block loudly instead of stalling.
  const hasHealthChecks = (config.health_checks ?? []).length > 0;
  ...
  if (agent === 'workflow') {
    ...
    return {
      prompt: buildOrchestrationPrompt(ids, workflow, maxRetries, ticketsPath, maxIterations, agentsPath, hasServices, gitConventions, hasHealthChecks),
  ...
  if (agent === 'feature') {
    ...
    return {
      prompt: buildFeaturePrompt(epicId, epic.data.title, children, workflow, maxRetries, ticketsPath, maxIterations, agentsPath, gitConventions, hasHealthChecks),
  ...
  if (agent === 'next') {
    ...
    return {
      prompt: buildNextStepPrompt(ticketId, workflow, ticketsDir, ticketsPath, agentsPath, hasHealthChecks),
  ...
  if (ids.length === 0) {
    ...
    // `assigned` is the static claim; a run.lock is the LIVE one (BOB-120). A
    // batch must skip both, or it hands an agent a ticket the app is already
    // running and the two overwrite each other on one branch.
    //
    // listTickets already carries each ticket's path, so the claim file is a
    // join and not a lookup. Going back through findTicket re-read the whole
    // board per ticket — 483ms on a 600-ticket board, for a fact already in hand.
    const claimOf = (t) => {
      // An unreadable lock must not stop work — a run.lock at mode 000 made
      // `bobby run plan` die with a raw EACCES. The tolerance lived in
      // ticketHasLiveRun and was lost when this path stopped calling it.
      try { return readTicketClaim(claimFileForTicket(t)); } catch { return null; }
    };
    const claims = new Map(stageTickets.map(t => [t.id, claimOf(t)]));
    const heldByOther = (t) => { const c = claims.get(t.id); return c !== null && !isOwnClaim(c); };
    const available = stageTickets.filter(t => !t.assigned && !heldByOther(t));
    if (available.length === 0) {
      if (stageTickets.length > 0) {
        const running = stageTickets.filter(heldByOther);
        if (running.length > 0) {
          // Name a holder, as a single-ticket refusal does — "N assigned" alone
          // sends someone hunting for an assignee that does not exist.
          const first = claims.get(running[0].id);
          // Same fallback claimedMessage uses: a record without a holder printed
          // a literal `undefined`, and naming the holder is the entire point.
          const who = (first && first.holder) || 'another run';
          throw new Error(
            `No available tickets in "${stage}" stage: ${running.length} already running ` +
            `(${running[0].id} — ${who}, started ${first.startedAt})` +
            (stageTickets.length > running.length ? `, ${stageTickets.length - running.length} assigned.` : '.')
          );
        }
    ...
    // BOB-090: with no live app there is nothing for bobby-test to exercise —
    // block each ticket loudly instead of launching agents that can only stall.
    if (reg.agentName === 'bobby-test' && !hasHealthChecks) {
      return {
        prompt: `The following ticket(s) are in testing, but ${testBlockInstruction('<id>')}\n\n` +
          `Block each of these tickets with that exact command and reason:\n` +
          batchIds.map(id => `- ${id}`).join('\n'),
        label: `${reg.label} — ${batchIds.length} ticket(s) in ${stage} (no live app)`,
      };
    }
  ...
  const ticketId = ids[0];
  if (!findTicket(ticketsDir, ticketId)) throw new Error(`Ticket ${ticketId} not found`);
  // BOB-090: same no-live-app gate for a single-ticket test run.
  if (reg.agentName === 'bobby-test' && !hasHealthChecks) {
    return {
      prompt: `Ticket ${ticketId} is in testing, but ${testBlockInstruction(ticketId)}`,
      label: `${reg.label} — ${ticketId} (no live app)`,
    };
  }
  // NO claim check here. buildPromptFor is a pure builder and the ORCHESTRATOR
  // calls it in-process for its own agents (orchestrator.js:449) — it already
  // holds the claim, so checking here refused every app-driven run. The guard
  // belongs at the CLI entry point, which is the one that represents a genuinely
  // separate operator; see assertTicketFree's callers in commands/run.js.
  return {
    prompt: buildSingleAgentPrompt(reg.agentName, ticketId, ticketsPath, agentsPath, hasServices, hasProduct, nextStageForAgent(reg.agentName, workflow), productDir),
...
/** Refuse a run on a ticket that already has one, naming the holder. */
export function assertTicketFree(ticketsDir, ticketId, known = null) {
  const ticket = known || findTicket(ticketsDir, ticketId);
  if (!ticket) return;
  let record = null;
  try { record = readTicketClaim(claimFileForTicket(ticket)); } catch { return; }
  // Our own claim is not a collision: an app-driven workflow runs stage after
  // stage against one workspace, and the claim covers the workspace.
  if (record && !isOwnClaim(record)) throw new Error(`${ticketId}: ${claimedMessage(record)}`);
}

/**
 * Build a prompt for running a full feature workflow (epic + children on one branch)
 */
export function buildFeaturePrompt(epicId, epicTitle, childTickets, workflow, maxRetries = 3, ticketsDir = '.bobby/tickets', maxIterations, agentsPath = '.claude/agents', gitConventions = {}, hasHealthChecks = true) {
  ...
  const epicDir = `${ticketsDir}/${resolveTicketFolder(ticketsDir, epicId)}`;
  ...
  const execSteps = execPipeline.map((s, i) => {
    ...
    return `${i + 1}. If stage is "${s.stage}":\n` +
      ...
      `     b) Load context: run \`bobby ticket view {TICKET_ID}\`\n` +
  ...
  const siblingPlanPaths = childTickets.map(t =>
    `  - \`${ticketsDir}/${t.dirname || resolveTicketFolder(ticketsDir, t.id)}/plan.md\``
  ).join('\n');
...
${pastPlanning.length > 0 ? `**Already past planning (read these for context):** ${pastPlanning.map(t => `\`${ticketsDir}/${t.dirname || resolveTicketFolder(ticketsDir, t.id)}/plan.md\``).join(', ')}` : ''}
...
2. Load context: run \`bobby ticket view {TICKET_ID}\` (resolves the folder even though your cwd is a worktree in a different tree than the board)
3. Read the existing sibling plans listed here for cross-ticket context:
${siblingPlanPaths}
...
6. After planning, verify the artifacts were written: \`bobby ticket view {TICKET_ID} --files\` lists both \`plan.md\` and \`test-cases.md\`
...
${!pastPlanning.length ? '' : `Read existing plans for context: ${pastPlanning.map(t => `\`${ticketsDir}/${t.dirname || resolveTicketFolder(ticketsDir, t.id)}/plan.md\``).join(', ')}`}
`;
...
4. After each agent completes, re-run \`bobby ticket view {TICKET_ID}\` and read the \`Stage:\` line to confirm the stage advanced.

5. If the stage advanced to the next workflow step, continue to the next agent.

6. If the stage went back to "building" (rejection), loop back and run bobby-build again. Maximum retries: ${maxRetries}. The build agent should read the rejection comment via \`bobby ticket view {TICKET_ID}\` (the \`## Comments\` section prints in the body) to understand what to fix. **On the 2nd retry failure**, run bobby-debug (follow \`${agentsPath}/bobby-debug.md\`) before the 3rd build attempt.
...
export function buildSprintPrompt(sprint, tickets, workflow, {
  ...
  const steps = buildSubagentSteps(workflow, { ticketsDir, agentsPath, planReadPath: sprintPlanPath, hasHealthChecks });
