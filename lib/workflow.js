...
export function buildNextStepPrompt(ticketId, workflow, ticketsDir, ticketsPath = '.bobby/tickets', agentsPath = '.claude/agents') {
  ...
  return buildSingleAgentPrompt(agent, ticketId, ticketsPath, agentsPath, false, nextStageForAgent(agent, workflow));
}
...
export function buildPromptFor(agent, ticketIds, ctx) {
  ...
  if (reg.promptSteps) {
    ...
    if (reg.cowork && ids.length > 0) {
      ...
      prompt = buildSingleAgentPrompt(reg.agentName, ticketId, ticketsPath, agentsPath, hasServices, nextStageForAgent(reg.agentName, workflow));
