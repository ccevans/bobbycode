...
export const BUILT_IN_WORKFLOWS = {
  ...
  // The data-model and architecture steps (after journeys, before features —
  // the feature map is cut against the data model, never the reverse) state
  // the entities and the forward architectural intent; the mockups step
  // (after the feature map locks, before blueprint) designs the v1 screens
  // FROM the personas/journeys. All three are optional — `--no-data-model`,
  // `--no-architecture` and `--no-mockups` filter them via omitStage, and
  // each gate accepts "skip".
  define: ['define-brief', 'define-personas', 'define-journeys', 'define-data-model', 'define-architecture', 'define-features', 'define-mockups', 'define-blueprint'],
...
export const STAGE_MAP = {
  ...
  'define-journeys': 'define-journeys',
  'define-data-model': 'define-data-model', 'define-architecture': 'define-architecture',
  'define-features': 'define-features',
  'define-mockups': 'define-mockups', 'define-blueprint': 'define-blueprint',
};

/**
 * A workflow without the named stage. Handoffs are computed from the returned
 * array (nextStageName / nextStageForAgent), so filtering a stage out is ALL
 * a skip flag has to do — the neighbouring steps hand to each other with no
 * further change. A no-op for any workflow that never had the stage.
 */
export function omitStage(workflow, stageName) {
  return workflow.filter(s => s.stage !== stageName);
}
