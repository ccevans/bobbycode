...
export function buildProfileFromDiscovery({ composeResult, classified, uiProject, profileName }) {
  ...
  // API port from web/api service
  if (classified.api) {
    const webPort = classified.api.ports.find(p => p.container === 3000 || p.container === 8080 || p.container === 5000);
    if (webPort) {
  ...
  if (classified.db) {
    ...
    if (dbPort) {
      profile.ports.postgres = dbPort.host;
    } else if (classified.db.ports.length > 0) {
      profile.ports.postgres = classified.db.ports[0].host;
    }
  }
