// lib/skills.js
// Studio per-project skill overlays. Bobby already layers a skill as
//   SKILL.md (shipped)  <-  SKILL.local.md (studio-level, yours).
// A studio adds a third, project-scoped tier so one project can reshape a skill and
...
export function projectSkillsRelDir(config) {
  if (config && config.studio && config._project) return path.join('.bobby', config._project, 'skills');
  return null;
}
...
export function repoTargetingClause(config) {
  if (!config || !config.studio) return '';
  ...
  return `\n\nMulti-repo project: read the ticket's \`repos\` frontmatter (falls back to the project's ` +
    `\`repos\` if unset). For each repo name, resolve its path from the \`repos:\` map in the studio ` +
