...
**Goal:** ${data.goal || '_Define the sprint goal._'}

**Branch:** \`${data.branch}\`

> The authoritative, ordered ticket list lives in \`sprint.yml\`.
> Edit this file with the cross-cutting context the sprint runner and agents should know.

## Sequencing rationale

_Why this order? Note dependencies between tickets so work lands safely._
...
_What this sprint explicitly does NOT cover._
`;
}
 ...
 */
export function createSprint(sprintsDir, { name, goal = '', pipeline = 'default', tickets = [], branchPrefix = 'feature' }) {
  if (!name || !name.trim()) {
