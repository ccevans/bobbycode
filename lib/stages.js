...
export const STAGES = [
  ...
  // epic. Brief → personas → journeys → data model → architecture → feature map
  // → mockups → blueprint, then the epic moves to planning for traceable
  // decomposition. Ordinary tickets skip these.
  'define-brief',
  'define-personas',
  'define-journeys',
  'define-data-model',
...
export const TRANSITIONS = {
  ...
  // Full words on purpose: `arch` is `bobby run arch`'s name, and `model`
  // invites the mockup/mockups class of near-neighbour trap. No shorthands.
...
export function stageColor(stage) {
  const colors = {
    ...
    'define-data-model': chalk.magentaBright,
    'define-architecture': chalk.magentaBright,
