...
const VALID_AREAS = ['security', 'reliability', 'operability', 'change-safety', 'product', 'data', 'revenue'];
...
export function loadPack(dir) {
  ...
  return {
    ...
    checks,
    roadmap,
    // When this version shipped — lets a lapsed Bobby Pro keep what it paid
    // for while newer packs stay behind the renewal.
    released: raw.released || null,
    // Paid packs carry `license: { pro: true }` (Bobby Pro) or their own
    // public key (sold standalone); free packs omit this entirely.
