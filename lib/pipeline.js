...
/**
 * Parse a pipeline definition (array of step names) into pipeline objects.
 */
...
/**
 * Resolve the pipeline to use, with this precedence:
 *   1. Explicit --pipeline flag (unless 'default')
 *   2. Ticket frontmatter `pipeline` field
 *   3. Config 'default' pipeline
 *   4. Built-in DEFAULT_PIPELINE
 *
 * @param {object} config     - readConfig() result
 * @param {string} name       - pipeline name from --pipeline flag (default: 'default')
 * @param {string|null} ticketPipeline - pipeline name from ticket frontmatter
 * @returns {Array<{stage: string, agent: string}>}
 */
export function resolvePipeline(config, name = 'default', ticketPipeline = null) {
  ...
  if (pipelines[effectiveName]) {
    const pipelineConfig = pipelines[effectiveName];
    return Array.isArray(pipelineConfig) ? parseSteps(pipelineConfig) : DEFAULT_PIPELINE;
  }
  if (effectiveName !== 'default') {
    const available = Object.keys(pipelines);
    available.unshift('default');
    throw new Error(`Unknown pipeline '${effectiveName}'. Available: ${available.join(', ')}`);
  }
  return DEFAULT_PIPELINE;
}

/**
 * List all pipeline names from config (including the implicit 'default').
 */
export function listPipelines(config) {
  const pipelines = config.pipelines || {};
  const names = new Set(['default', ...Object.keys(pipelines)]);
  return [...names];
}
 ...
 */
export function buildOrchestrationPrompt(ticketIds, pipeline, maxRetries = 3, ticketsDir = '.bobby/tickets', maxIterations = 20, agentsPath = '.claude/agents', hasServices = false, gitConventions = {}) {
  const ids = Array.isArray(ticketIds) ? ticketIds : [ticketIds];
  const ticketBranchPrefix = gitConventions.ticket_branch_prefix || 'tkt';
  const firstStage = pipeline[0]?.stage || 'planning';
  ...
  const steps = pipeline.map((s, i) => {
    ...
    return `${i + 1}. If stage is "${s.stage}":${preGate}\n` +
      `     ${preGate ? 'b' : 'a'}) Claim: \`bobby ticket assign {TICKET_ID} ${s.agent}\`\n` +
      ...
      `         3. When done, move: \`bobby ticket move {TICKET_ID} ${next}\`.\n` +
...
2. **Pre-flight stage gates** — handle non-pipeline stages before launching any subagent:
   ...
   - If stage is **"backlog"**: advance to the first pipeline stage now — \`bobby ticket move {TICKET_ID} ${firstStage}\` — then re-read \`ticket.md\` and continue with the new stage.
...
5. If the stage went back to "building" (rejection), the subagent rejected the work. Loop back and launch a bobby-build subagent again. Maximum retries: ${maxRetries}. **On the 2nd retry failure**, launch a bobby-debug subagent (prompt: "Follow \`${agentsPath}/bobby-debug.md\` on ticket {TICKET_ID}") before the 3rd build attempt — the debug agent's structured investigation often breaks deadlocks that repeated build attempts cannot.
...
8. If the stage is anything else not covered above (e.g., outside the configured pipeline), log a warning and move to the next ticket — do not silently skip.
...
export function buildPromptFor(agent, ticketIds, ctx) {
  ...
  if (agent === 'pipeline') {
    ...
    return {
      prompt: buildOrchestrationPrompt(ids, pipeline, maxRetries, ticketsRelDir, maxIterations, agentsPath, hasServices, gitConventions),
  ...
  if (agent === 'feature') {
    ...
    return {
      prompt: buildFeaturePrompt(epicId, epic.data.title, children, pipeline, maxRetries, ticketsRelDir, maxIterations, agentsPath, gitConventions),
 ...
 */
export function buildFeaturePrompt(epicId, epicTitle, childTickets, pipeline, maxRetries = 3, ticketsDir = '.bobby/tickets', maxIterations, agentsPath = '.claude/agents', gitConventions = {}) {
...
5. If the stage advanced to the next pipeline step, continue to the next agent.
...
7. If the stage is "shipping", this ticket's pipeline is complete.
 ...
 * groups arbitrary, unrelated tickets. Each ticket runs its own full pipeline
 ...
 * @param {Array}  pipeline - resolved pipeline array [{ stage, agent }]
 * @param {object} opts     - { maxRetries, ticketsDir, maxIterations, agentsPath, hasServices, sprintPlanPath }
 */
export function buildSprintPrompt(sprint, tickets, pipeline, {
  maxRetries = 3,
  ...
  if (!maxIterations) {
    maxIterations = Math.max(ids.length * pipeline.length * 2, 10);
  }
  const firstStage = pipeline[0]?.stage || 'planning';
  ...
  const planRead = sprintPlanPath
    ? `\n         2. Read \`${sprintPlanPath}\` for sprint-wide context (sequencing, shared decisions, out-of-scope).`
    : '';
  const planOffset = sprintPlanPath ? 1 : 0;
  ...
  const steps = pipeline.map((s, i) => {
    ...
    return `${i + 1}. If stage is "${s.stage}":${preGate}\n` +
      ...
      `         ${2 + planOffset}. CRITICAL: Read and follow \`${agentsPath}/${s.agent}.md\` end-to-end — do NOT skip it or improvise your own.\n` +
...
4. If the stage advanced to the next pipeline step, continue to the next agent for the same ticket.

5. If the stage went back to "building" (rejection), loop back and launch a bobby-build subagent again. Maximum retries: ${maxRetries}. **On the 2nd retry failure**, launch a bobby-debug subagent (prompt: "Follow \`${agentsPath}/bobby-debug.md\` on ticket {TICKET_ID}") before the 3rd build attempt.
...
8. If the stage is anything else not covered above (e.g., outside the configured pipeline), log a warning and skip to the next ticket — do not silently move on.
