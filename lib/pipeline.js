 ...
 */
/**
 * Shared fragment: per-stage subagent-launch steps. Used by both the pipeline
 ...
 */
function buildSubagentSteps(pipeline, { ticketsDir, agentsPath, planReadPath = '' }) {
 ...
 * Identical for pipeline and sprint orchestration.
 */
function buildPreflightGates(firstStage) {
  return `**Pre-flight stage gates** — handle non-pipeline stages before launching any subagent:
 ...
 * 2nd failure. Identical for pipeline and sprint orchestration.
 */
...
export function buildOrchestrationPrompt(ticketIds, pipeline, maxRetries = 3, ticketsDir = '.bobby/tickets', maxIterations = 20, agentsPath = '.claude/agents', hasServices = false, gitConventions = {}) {
  ...
  const steps = buildSubagentSteps(pipeline, { ticketsDir, agentsPath });
...
export function buildSprintPrompt(sprint, tickets, pipeline, {
  ...
  const steps = buildSubagentSteps(pipeline, { ticketsDir, agentsPath, planReadPath: sprintPlanPath });
