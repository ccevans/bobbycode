 ...
 */
export function buildBatchStagePrompt(agent, ticketIds, ticketsDir = '.bobby/tickets', isolation = 'none') {
...
For each ticket, launch a subagent with isolation: "worktree" and this prompt:
  ...
  4. Follow the instructions in \`.claude/agents/${agent}.md\`.
...
| Ticket | Commit | Files Changed | +/- Lines |
|--------|--------|---------------|-----------|
| {ID} | {short SHA} | {N} files | +{added} / -{removed} |

(Get stats from git log --oneline --stat on the feature branch.)

## Tests

| Command | Result |
|---------|--------|
| {test command} | {pass / fail / skipped: reason} |
...
{Any blockers, integration issues, bundled tickets, or patterns observed. Omit section if none.}
\`\`\``;
}
