...
5. If the stage went back to "building" (rejection), the agent rejected the work. Loop back and run bobby-build again. Maximum retries: ${maxRetries}. The build agent should read the rejection comment in ticket.md to understand what to fix. **On the 2nd retry failure**, run bobby-debug (follow \`.claude/agents/bobby-debug.md\`) before the 3rd build attempt — the debug agent's structured investigation often breaks deadlocks that repeated build attempts cannot.
 ...
 */
export function buildSecurityPrompt(ticketId, ticketsDir = '.bobby/tickets') {
...
4. Follow the instructions in \`.claude/agents/bobby-security.md\`.
 ...
 */
export function buildDebugPrompt(ticketId, ticketsDir = '.bobby/tickets') {
...
3. Follow the instructions in \`.claude/agents/bobby-debug.md\`.
 ...
 */
export function buildDocsPrompt(ticketsDir = '.bobby/tickets') {
  return `Run the bobby-docs agent to update project documentation.

1. Follow the instructions in \`.claude/agents/bobby-docs.md\`.
 ...
 */
export function buildPerfPrompt(ticketsDir = '.bobby/tickets') {
  return `Run the bobby-perf agent for performance benchmarking.

1. Follow the instructions in \`.claude/agents/bobby-perf.md\`.
...
/**
 * Build a prompt for the post-deploy canary agent
 */
export function buildCanaryPrompt(ticketsDir = '.bobby/tickets') {
  return `Run the bobby-canary agent for post-deploy verification.

1. Follow the instructions in \`.claude/agents/bobby-canary.md\`.
2. Read \`.bobbyrc.yml\` for \`production_url\` and \`canary_pages\` config.
3. Navigate to each canary page and verify:
   - HTTP 200 response
   - Page renders within 5 seconds
   - No JavaScript console errors
   - No failed network requests
4. Screenshot each page for the record.
5. Save results to \`.bobby/canary/\`.
...
/**
 * Build a prompt for the groom agent (backlog grooming)
 */
export function buildGroomPrompt(ticketsDir = '.bobby/tickets') {
  return `Run the bobby-groom agent for backlog grooming.

1. Run \`bobby list backlog --sort oldest\` to see the full backlog.
2. For each backlog ticket, read \`${ticketsDir}/{ID}*/ticket.md\` to understand scope and quality.
3. Identify:
   a) Duplicate or overlapping tickets — recommend consolidation
   b) Tickets that are too vague — missing acceptance criteria or unclear description
   c) Tickets that are no longer relevant — recommend archiving via \`bobby archive {ID}\`
   d) Priority mismatches — high-priority items sitting idle, low-value items marked high
4. Produce a grooming report with specific recommendations:
   - Tickets to merge (list pairs/groups with rationale)
   - Tickets to archive (\`bobby archive {ID} ...\`)
   - Tickets to reprioritize (\`bobby update {ID} --priority {new}\`)
   - Tickets needing refinement (missing AC, vague description)
5. Present the report to the user and ask for confirmation before executing any changes.`;
}

/**
 * Build a prompt for the vet agent (design interrogation before planning)
 */
export function buildVetPrompt(ticketsDir = '.bobby/tickets') {
  return `Run the bobby-vet agent to vet an idea or ticket design before planning.

1. Follow the instructions in \`.claude/agents/bobby-vet.md\`.
...
5. If the stage went back to "building" (rejection), loop back and run bobby-build again. Maximum retries: ${maxRetries}. The build agent should read the rejection comment in ticket.md to understand what to fix. **On the 2nd retry failure**, run bobby-debug (follow \`.claude/agents/bobby-debug.md\`) before the 3rd build attempt.
