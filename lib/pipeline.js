 ...
 */
export function buildOrchestrationPrompt(ticketIds, pipeline, maxRetries = 3, ticketsDir = '.bobby/tickets', maxIterations = 20, runsDir = '.bobby/runs', agentsPath = '.claude/agents') {
  ...
  const steps = pipeline.map((s, i) => {
    ...
    return `${i + 1}. If stage is "${s.stage}":\n` +
      ...
      `     c) Follow the instructions in \`${agentsPath}/${s.agent}.md\`\n` +
 ...
 */
export function buildSingleAgentPrompt(agent, ticketId, ticketsDir = '.bobby/tickets', agentsPath = '.claude/agents') {
...
3. Follow the instructions in \`${agentsPath}/${agent}.md\`.
...
For each ticket, launch a subagent with isolation: "worktree" and this prompt:
  ...
  4. Follow the instructions in \`${agentsPath}/${agent}.md\`.
 ...
 */
export function buildFeaturePrompt(epicId, epicTitle, childTickets, pipeline, maxRetries = 3, ticketsDir = '.bobby/tickets', maxIterations, runsDir = '.bobby/runs', agentsPath = '.claude/agents') {
