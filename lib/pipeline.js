 ...
 */
export function buildOrchestrationPrompt(ticketIds, pipeline, maxRetries = 3, ticketsDir = '.bobby/tickets', maxIterations = 20, runsDir = '.bobby/runs', agentsPath = '.claude/agents', hasServices = false) {
  ...
  const steps = pipeline.map((s, i) => {
    ...
    return `${i + 1}. If stage is "${s.stage}":${preGate}\n` +
      ...
      `     ${preGate ? 'c' : 'b'}) **Launch a subagent** (using the Agent tool) with this prompt:\n` +
      `        "You are ${s.agent} working on ticket {TICKET_ID}.\n` +
      `         1. Read \`${ticketsDir}/{TICKET_ID}*/ticket.md\` to load context.\n` +
      `         2. Follow the instructions in \`${agentsPath}/${s.agent}.md\`.\n` +
