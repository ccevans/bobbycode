 ...
 */
export function buildOrchestrationPrompt(ticketIds, pipeline, maxRetries = 3, ticketsDir = '.bobby/tickets', maxIterations = 20) {
...
2. Write a run log to \`.bobby/runs/\` (create the directory if it doesn't exist). Filename: \`run-{YYYYMMDD-HHmmss}.md\` using the current date/time. Format:
...
4. When complete, update the ticket stage using \`bobby move\` (auto-clears assignment).
...
For each ticket, launch a subagent with this prompt:
  ...
  1. Read \`${ticketsDir}/{ID}*/ticket.md\` to load the ticket context.
  2. Follow the instructions in \`.claude/agents/${agent}.md\`.
  3. When complete, update the ticket stage using \`bobby move\`.
 ...
 */
export function buildFeaturePrompt(epicId, epicTitle, childTickets, pipeline, maxRetries = 3, ticketsDir = '.bobby/tickets', maxIterations = 20) {
...
For each ticket that needs planning (in order):

1. Move to planning if in backlog: \`bobby move {TICKET_ID} plan\`
2. Claim: \`bobby assign {TICKET_ID} bobby-plan\`
3. Read the ticket's \`ticket.md\` for context
4. Read all existing sibling plan.md files for cross-ticket context:
${siblingPlanPaths}
5. Read \`${featurePlanPath}\` if it exists (from a prior run or earlier ticket in this phase)
6. Follow \`.claude/agents/bobby-plan.md\` — the skill will detect Feature-Aware Refine Mode when it sees the ticket has a parent epic
7. After planning, verify \`plan.md\` and \`test-cases.md\` were created in the ticket folder
8. Move to building: \`bobby move {TICKET_ID} build\`
9. Verify: \`git status --short\` — no uncommitted source files
...
5. Write a feature run log to \`.bobby/runs/\` (create the directory if it doesn't exist). Filename: \`feature-${epicId.toLowerCase()}-{YYYYMMDD-HHmmss}.md\`. Format:
