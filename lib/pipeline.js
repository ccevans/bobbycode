...
2. **Pre-flight stage gates** — handle non-pipeline stages before launching any subagent:
   ...
   - If stage is **"backlog"**: advance to the first pipeline stage now — \`bobby move {TICKET_ID} ${firstStage}\` — then re-read \`ticket.md\` and continue with the new stage.
...
export function buildSprintPrompt(sprint, tickets, pipeline, {
  ...
  const steps = pipeline.map((s, i) => {
    ...
    return `${i + 1}. If stage is "${s.stage}":${preGate}\n` +
      `     ${k.claim}) Claim: \`bobby assign {TICKET_ID} ${s.agent}\`\n` +
      ...
      `         ${3 + planOffset}. When done, move: \`bobby move {TICKET_ID} ${next}\`.\n` +
