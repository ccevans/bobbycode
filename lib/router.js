...
export const CAPABILITIES = [
  { when: 'implement / add / build / change / fix a specific thing in this project', do: 'bobby go "<the task, restated>"', note: 'creates a ticket and runs the full build pipeline' },
...
2. If it's a concrete change to *this* project ("add…", "fix…", "make…", "build…") and nothing more specific fits, treat it as build work: \`bobby go "${request}"\`.
3. **Resolving \`<id>\`** for the "needs a ticket" capabilities:
   ...
   - If there's no ticket yet (a fresh symptom like "the login button does nothing", or a one-off "audit for leaked secrets"), the simplest correct route is \`bobby go "${request}"\` — it creates the ticket and runs the pipeline. Only reach for \`bobby run <verb>\` directly when a matching ticket already exists.
