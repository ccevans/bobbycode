...
const DEFAULTS = {
  ...
  // PER-AGENT MODELS, empty on purpose. Bobby's own opinion about which model
  // each stage wants lives with each agent (`tier` in lib/agent-registry.js)
  // and is applied in lib/models.js. Seeding a value here would make "the user
  // asked for this" indistinguishable from "nobody said", which is the same
  // reasoning that keeps `permission_mode` and `executor` unset below.
  models: {},
  dashboard: {
    port: 7777,
    ...
    //
    // executor_flavor is unset for the same reason, and does the same job one
    // level up (BOB-137): `executor` names the BINARY — a bare name or an
    // absolute path — while `executor_flavor` names the CLI dialect whose flags
    // drive it. Left unset, the flavor is inferred from the binary's own
    // basename (`/opt/bin/opencode` -> opencode), and a binary Bobby cannot
    // place is REFUSED rather than driven with claude's flags. Setting it here
    // would defeat that inference for every project at once, and would make a
    // deliberate `executor_flavor: claude` indistinguishable from nobody
    // saying. It is the escape hatch for a wrapper whose name says nothing
    // about the CLI underneath it.
  },
};
...
function readBaseConfig(rootDir) {
  ...
  merged.models = { ...DEFAULTS.models, ...(parsed.models || {}) };
...
export function writeConfigCommented(rootDir, config) {
  ...
  lines.push('# Options: claude-code | cursor | cline | codex | agents-md | copilot | opencode');
  ...
  if (!config.workflows && !config.pipelines) {
    ...
    lines.push('# secure, quick. Add or override your own here, then: bobby run <name> <ticket>');
  ...
  lines.push("# Models, per stage. Bobby already picks one per agent by kind of work —");
  lines.push("# judgment (opus) for plan/review/security/design-check, execution (sonnet)");
  lines.push("# for build/test/docs, bookkeeping (haiku) for performance/watchdog. Name a");
  lines.push("# stage here to override it, `default:` to override all of them, or");
  lines.push("# `inherit` to pass no model at all and let the harness decide.");
  lines.push("#");
  lines.push("# The shipped defaults are ALIASES, so they only apply on the claude");
  lines.push("# executor. cursor-agent / codex / opencode take full model names — until you");
  lines.push("# name them here, those projects run with no model flag, as they always have.");
  lines.push("# models:");
  lines.push("#   default: sonnet");
  lines.push("#   plan: opus");
  lines.push("#   build: sonnet");
  lines.push("#   review: opus");
  lines.push("#   test: haiku");
  lines.push("#   watchdog: inherit");
  lines.push('');
  ...
  lines.push('#   executor: claude                  # claude | cursor-agent | codex | opencode');
  lines.push('#                                     # (default: derived from target) — or an');
  lines.push('#                                     # /abs/path/to/a/binary, never a relative one.');
  lines.push('#   executor_flavor: claude           # Which of those four dialects the binary');
  lines.push('#                                     # above speaks. A bare flavor name only,');
  lines.push('#                                     # never a path. Unset, it is inferred from');
  lines.push("#                                     # the binary's basename (/opt/bin/opencode ->");
  lines.push('#                                     # opencode) and a binary Bobby cannot place is');
  lines.push('#                                     # REFUSED, not driven with the wrong flags. Set');
  lines.push('#                                     # it for a wrapper, or any binary whose name');
  lines.push('#                                     # says nothing about the CLI underneath.');
  lines.push('#   model: null                       # One model for EVERY agent. Overrides');
  lines.push('#                                     # the per-stage defaults above, and is');
  lines.push('#                                     # itself overridden by models:. For');
  lines.push('#                                     # names: cursor-agent --list-models');
