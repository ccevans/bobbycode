...
export function writeConfigCommented(rootDir, config) {
  ...
  lines.push('# Options: claude-code | cursor | codex | cline | agents-md');
  ...
  // Project-wide default workflow. Only written when it isn't the built-in
  // default, so a normal web project's config stays uncluttered.
  if (config.default_workflow && config.default_workflow !== 'default') {
    lines.push('# Workflow used when you do not pass --workflow.');
    if (config.default_workflow === 'library') {
      lines.push('# This project has no dev server or health checks, so Bobby picked');
      lines.push('# `library` — it ends at review instead of the live-app `test` stage,');
      lines.push('# which would have nothing to observe. Review runs your test suite.');
    }
  ...
  if (!config.workflows && !config.pipelines) {
    ...
    lines.push('#   library: [plan, build, review]            # no live app to test');
    lines.push('#   library-secure: [plan, build, security, review]');
    lines.push('');
  }
