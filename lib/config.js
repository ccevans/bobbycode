...
export function readConfig(rootDir) {
  ...
  // v2 workspace: a single workspace holds many projects, each with its own
  // board under .bobby/<project>/. Resolve the active project and cascade its
  // config over the workspace defaults. v1 projects (no `layout` key) are
  // untouched and fall straight through.
  if (merged.layout === 'v2') return applyProjectContext(rootDir, merged);

  return merged;
}

/**
 * v2 only. List the projects in a workspace — every .bobby/<name>/ that carries
 * its own .bobbyrc.yml.
 */
export function listWorkspaceProjects(rootDir) {
...
/**
 * v2 only. Which project a command targets. Precedence:
...
export function resolveActiveProject(rootDir, explicit) {
  ...
  const all = listWorkspaceProjects(rootDir);
  if (all.length === 1) return all[0];
  return null;
}

function applyProjectContext(workspaceRoot, workspaceCfg) {
  const project = resolveActiveProject(workspaceRoot);
  workspaceCfg.repo_group = workspaceCfg.repos || {};   // name -> {path, stack, commands}
  workspaceCfg._workspace = workspaceCfg.workspace || path.basename(workspaceRoot);
  if (!project) { workspaceCfg._project = null; return workspaceCfg; }

  const pcPath = path.join(workspaceRoot, '.bobby', project, CONFIG_FILE);
  const pc = fs.existsSync(pcPath) ? (YAML.parse(fs.readFileSync(pcPath, 'utf8')) || {}) : {};
  const merged = { ...workspaceCfg, ...pc };
  merged.git_conventions = { ...(workspaceCfg.git_conventions || {}), ...(pc.git_conventions || {}) };
  merged.dashboard = { ...(workspaceCfg.dashboard || {}), ...(pc.dashboard || {}) };
  merged.workflows = { ...(workspaceCfg.workflows || {}), ...(pc.workflows || {}) };
  merged.layout = 'v2';
  merged.repo_group = workspaceCfg.repos || {};
  merged.project_repos = pc.repos || [];                 // subset of the group this project uses
  merged.ticket_prefix = pc.prefix || pc.ticket_prefix || workspaceCfg.ticket_prefix || 'TKT';
  merged._project = project;
  merged._workspace = workspaceCfg._workspace;
...
/**
 * v2 only. Absolute path to a repo in the workspace group. Relative paths in
 * the group (the default `repos/<name>` layout) resolve against the workspace
 * root; absolute paths (an external repo you didn't relocate) pass through.
 */
export function resolveRepoPath(workspaceRoot, config, name) {
  const group = config.repo_group || config.repos || {};
  const entry = group[name];
  if (!entry) throw new Error(`Repo '${name}' is not in the workspace group`);
...
export function writeConfigCommented(rootDir, config) {
  ...
  lines.push('# Options: claude-code | cursor | cline');
  ...
  lines.push('#   permission_mode: null             # bypassPermissions | acceptEdits | plan');
  lines.push('#                                     # Unset = each CLI\'s own default, which');
  lines.push('#                                     # may stop to ask before editing files.');
  lines.push('');
...
export function resolveTicketsDir(root, config) {
  if (config && config.layout === 'v2') return v2BoardDir(root, config, 'tickets');
...
// v2: the active project's board lives at .bobby/<project>/<leaf>. If no project
// is selected we can't know which board — fail with an actionable message.
function v2BoardDir(root, config, leaf) {
  if (!config._project) {
    const all = listWorkspaceProjects(root);
...
export function resolveSessionsDir(root, config) {
  if (config && config.layout === 'v2') return v2BoardDir(root, config, 'sessions');
  const mainRoot = findMainWorktreeRoot(root);
  const effectiveRoot = (mainRoot && mainRoot !== root) ? mainRoot : root;
