...
export function detectProjectContext(rootDir) {
  ...
  // .github/copilot-instructions.md is DETECTION only — the scaffold never
  // writes it (lib/targets/copilot.js header: instruction files are combined,
  // so duplicating rules there would drift).
  const rulesFiles = ['CLAUDE.md', '.claude/CLAUDE.md', 'AGENTS.md', '.cursor/rules', '.clinerules/rules.md', '.github/copilot-instructions.md'];
