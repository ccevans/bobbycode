...
import { deriveDefaultWorkflow } from './workflow.js';
...
export function createProject(idea, { dir, stack: stackName = 'node', cwd = process.cwd() } = {}) {
  ...
  // BOB-090: same derivation as `bobby init` — a stack with no live app
  // defaults to the `library` workflow, written visibly into .bobbyrc.yml.
  ...
  try {
    ...
    committed = false;
    // git's OWN words, not just "Command failed: git commit -m ...". With
    // stdio:'pipe' the child's stderr lands on `e.stderr` while `e.message`
    // carries only the command line, so keeping the first line alone told a
    // user whose commit was refused nothing about why — and told CI nothing
    // either, which is how this stayed unexplained (TKT-072).
    const summary = String(e.message || '').split('\n')[0];
    const reason = String(e.stderr || '').trim().replace(/\s+/g, ' ');
    commitError = reason ? `${summary}: ${reason}` : summary;
  }
