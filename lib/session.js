...
/**
 * Walk a session's move entries and collect elapsed time per stage.
 * Returns { stage: [elapsedMs, ...] }. Shared by sessionSummary and the
 * weekly retro's cross-session aggregation.
 */
export function stageTimingsFromMoves(moves) {
  const stageTimings = {};
...
export function sessionSummary(sessionsDir, sessionId) {
  ...
  const stageTimings = stageTimingsFromMoves(moves);
