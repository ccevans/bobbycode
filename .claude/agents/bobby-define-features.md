...
Follow **the Feature Map step (Step 6)** of `.claude/skills/bobby-define/SKILL.md` in full, including the lock-and-handoff sequence.
...
2. Read `.bobby/product/DATA-MODEL.md` **when present** (its stage can be skipped) and cut the map against it: a Must feature needing an entity absent from the model amends the model (with a Changelog line) or goes out — it never invents an entity locally.
3. Build the map per the skill's skeleton: `F<j>.<seq>` IDs keyed to journeys, journey-step and persona columns, MoSCoW. **v1 = the Must rows.** Every Never row cites a brief Non-goal (`F0.x` for cross-cutting rows).
4. ⛳ Gate (verbatim): "The **Must** column is the whole of v1. Strike one thing from it — or tell me why nothing can go."
5. After the gate: set `Locked/Status: approved` on every artifact present in `.bobby/product/` (six when nothing was skipped); `git add .bobby/product .bobby/decisions.yaml && git commit -m "product: definition locked for {EPIC}"` — the ADRs land with the definition; comment (`--by bobby-define-features`) with the one-line summary.
