...
> definition: brief, personas, journeys, feature map, and a blueprint you can
...
## Step 5: The Blueprint — the glimpse before building
...
Never). Then commit the page alongside the artifacts.
...
Each stage comments on the epic when it reaches its gate (commands shown per step). The epic's stage tracks progress through the pipeline (`define-brief` → … → `define-features` → `planning`), so `bobby brief` and the app always know where definition stands.
