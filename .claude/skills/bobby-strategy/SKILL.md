...
1. **Check learnings** — Read `.claude/skills/bobby-strategy/learnings.md` + `.claude/skills/bobby-strategy/learnings.local.md`
2. **Read the ticket** — title, description, acceptance criteria, priority, type, area
3. **Survey the backlog** — `bobby ticket list backlog --sort priority` to understand relative positioning
4. **Check in-flight work** — `bobby ticket list planning`, `bobby ticket list building` to see current capacity
...
1. State the decision (approve/defer/kill)
2. Show the complete scorecard
3. Rate your confidence 1-10
4. If confidence is below 7: DEFER (not kill) and note what information would raise confidence
...
1. Run `bobby ticket list backlog --sort priority` to get all backlog tickets
2. Evaluate each ticket through the full framework above
3. Rank all tickets by total score
4. Apply decisions based on thresholds
