...
1. **Check learnings** — Read `.claude/skills/bobby-debug/learnings.md` + `.claude/skills/bobby-debug/learnings.local.md`
2. Read the ticket's `ticket.md` — understand the expected vs actual behavior
3. Read rejection comments — understand what was tried and what failed
...
- For each hypothesis, trace the data flow through the code:
  ...
  - Use targeted `console.log` / `print` / debugger statements if needed (remove before committing)
...
- If you discover other issues, file them: `bobby ticket create -t "Found during debug: {issue}" --type bug`
...
1. Commit the fix: `git add` changed files, `git commit` with `TKT-{ID}: fix — {root cause summary}`
2. Remove any debug logging (console.log, print statements) added during investigation
3. Run full test suite and lint — show output as evidence
4. Add comment: `bobby ticket comment {ID} --by bobby-debug "Root cause: {explanation}. Fix: {what was changed}"`
5. If pattern is worth remembering: `bobby learn bobby-debug "pattern" "description"` and `bobby retro {ID} "pattern"`
...
2. Is there a test that would catch this regression?
3. Did you remove all debug logging?
4. Did you ONLY change files related to the bug?
5. Do all tests pass? Show the output.
</self_check>

## If Something Goes Wrong

- Cannot reproduce: `bobby ticket move {ID} block "Cannot reproduce: {steps tried}"`
- Fix requires architectural change: `bobby ticket move {ID} block "Root cause identified but fix requires {scope description}. Recommend new ticket."`
- 3 attempts exhausted: `bobby ticket move {ID} block "Debug: 3 attempts failed. Analysis: {summary of hypotheses and results}"`
