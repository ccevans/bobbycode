...
**Goal:** Build Bobby, an npm CLI that gives structure to AI-assisted development — a generic, configurable ticketing and workflow system for AI-powered coding.
...
> **Origin note:** Bobby was originally extracted from a project-specific bash script (`ticket.sh`). References to that script in this plan doc indicate the behavior source. All project-specific values have been replaced with configurable `.bobbyrc.yml` fields.
...
- [ ] **Create a new repo/worktree** for Bobby (standalone project)
...
These are the generic skill templates. Each `.ejs` template receives the config object and renders with the user's project-specific values.
...
This is the main rules template (`CLAUDE.md`). Key template variables:
...
The CLAUDE.md template should include all role instructions (PM, Engineer, QE, Refiner, Reviewer) with process logic using templated values.
...
- Replaces hardcoded skill paths with target-specific `.claude/skills/`
...
**Skill templates:**
- `work-tickets` — ticket execution workflow
- `qe` — quality engineering / testing
- `peer-review` — code review
- `refine-tickets` — ticket refinement
- `release-tickets` — release management
- `ideate` — ideation / brainstorming
