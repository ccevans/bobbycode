// test/lib/workspace.test.js — v2 workspace/project resolution.
import fs from 'fs';
import path from 'path';
import os from 'os';
import {
  readConfig, resolveTicketsDir, resolveActiveProject,
  listWorkspaceProjects, resolveRepoPath,
} from '../../lib/config.js';
import { execSync } from 'child_process';
import {
  upgradeToV2, createProject, setActiveProject, addRepo, readProjectConfig,
  setupRepos, readRawWorkspaceConfig,
...
describe('v2 workspace', () => {
  ...
  test('upgradeToV2 marks layout v2 and scaffolds repos/', () => {
    expect(readConfig(root).layout).toBe('v2');
  ...
  test('createProject writes a project with its own board and prefix', () => {
    ...
    expect(listWorkspaceProjects(root)).toContain('robinoffer');
  ...
  test('createProject rejects a repo not in the group', () => {
    expect(() => createProject(root, 'x', { repos: ['nope'] })).toThrow(/not in the workspace group/);
  });
  ...
  test('readConfig cascades project over workspace (prefix + board dir)', () => {
    createProject(root, 'robinoffer', { prefix: 'RO', repos: ['app'] });
    setActiveProject(root, 'robinoffer');
  ...
  test('repo url is recorded, and workspace setup restores a missing repo from it', () => {
    ...
    try {
      ...
      expect(readRawWorkspaceConfig(root).repos.cloned.url).toBe(remote);
  ...
  test('workspace setup reports a local-only repo (no url) as missing', () => {
    const local = fs.mkdtempSync(path.join(os.tmpdir(), 'bobby-local-'));   // no git origin
    try {
      addRepo(root, 'localonly', local);
      expect(readRawWorkspaceConfig(root).repos.localonly.url).toBeUndefined();
  ...
  test('resolveRepoPath resolves relative under the workspace, passes absolute through', () => {
    const cfg = readConfig(root);
    ...
    try {
      addRepo(root, 'ext', external);   // an existing repo outside the workspace (opt-out)
